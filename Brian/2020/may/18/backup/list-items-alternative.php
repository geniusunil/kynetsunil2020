...
{        
      ...
      $price_altrnative[$listitem] =  intval(get_field('price_starting_from', $listitem));
...
$top3_alternative = get_the_title($top1_alternative[2]);
  ...
  $list_item = get_field('add_to_list', $mainId, false);
        ...
        foreach($all_item_result as $ii)
        {
            ...
            $all_price_list[$ii] = intval(get_field( 'price_starting_from', $ii, false));
        ...
        $ind_price= $all_price_sum / count($all_item_id);
        $industry_fs = round($ind_price,2);

        $itemprice = intval(get_field( 'price_starting_from', $mainId));
...
if(!empty($two_altr))
{
    $lis_altrntiave = implode(",", $two_altr );
...
// echo "item fs $item_fs industry fs $industry_fs";
        ...
        //     print_r($noOflistitems);
        //    $item_pos = array_search($mainId, $noOflistitems);
        // break;

        // }
        // 
        // echo  "<br>noum".$item_pos;
        ...
        foreach($all_item_result as $post_ids)
        {
            ...
            $sorted_altrntve_list[$post_ids] = sort_features($item_feature_lists, $features_ids); 
        }
        ...
        foreach($res as $resultt){
            //     echo "result_123";
            //    echo $resultt;
            $price_all_item = get_field( 'price_starting_from', $resultt);
         
            if($itemprice > $price_all_item ){
                //    echo "abc";
            ...
            if(!empty($listana[1])){
                $show = 'or '. get_the_title($listana[1]).'';
            }
                $faq7 ="If $three[0] is what is most important to you then you might want to consider ". get_the_title($listana[0])." ". $show."" ;
    ...
    // echo "</pre>";
         ...
         foreach($rating_alternative as $key =>$rat){
            ...
            if($item_ease > $list){
                ...
                $faq_9 = ''.$item_name.' is the highest ease of use then alternative item.';
            }
        ...
        //   $faq_9 = 'If you are after a more user-friendly alternative to $item_name then you might want to consider '.$all_item_ease[0].'
        //   $ease_item  which as a higher ease of use rating than $item_name'
