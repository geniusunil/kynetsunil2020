      ...
      success: function (data)
      {
        ...
        for (const award of awards) {
          award.addEventListener('click', function(event) {
            ...
            var singular_title=event.target.getAttribute("singular-title").toUpperCase();
            var ranking_quota=event.target.getAttribute("quota").toUpperCase();
            var img = new Image();
            ...
            img.onload = function(){
              ...
              wrapText(ctx,singular_title,canvas.width*12/50,canvas.height*105/265,canvas.width/2,txtheight);
            ...
            function wrapText(context, text, x, y, maxWidth, lineHeight) { //https://www.html5canvastutorials.com/tutorials/html5-canvas-wrap-text-tutorial/
              ...
              console.log({half});
              line1= text.substring(0, half);
              line2= text.substring(half+2);
              console.log({line1});
              console.log({line2});
              ...
              for(var n = 0; n < words.length; n++) {
               ...
               /*  console.log({testWidth});
                ...
                if (testWidth > maxWidth && n > 0) {
                  metricsLine = context.measureText(line);
                  lineWidth = metricsLine.width;
                  xpluscenterpadding = (maxWidth-lineWidth)/2;
                  context.fillText(line, x+xpluscenterpadding, y);
                  line = words[n] + ' ';
                  y += lineHeight;
                }
                else {
                  line = testLine;
                  // x=x+(maxWidth-testWidth)/2;
                }
              }
              metricsLine = context.measureText(line);
              lineWidth = metricsLine.width;
