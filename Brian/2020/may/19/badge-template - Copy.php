    ...
    <div id="topsfbadges">
        <div class="design">
            ...
            <div>
                ...
                <div id="awardName" class="col-md-4">
    ...
    jQuery.ajax(
                    ...
                    '</div>';
                                ...
                                data['posts'].forEach(function(entry) {
                                  // console.log({entry});
                                  document.getElementById("myDropdown2").innerHTML += ("<span<span<span style='display: block;' post-id="+entry['ID']+" >"+entry['post_title']+"</span>");
...
//   alert("hi");
    ...
    post_title = event.target.innerText;
    document.getElementById("awardName").innerHTML =
    '<div class="dropdown">'+
                    '<button id="selectAward" onclick="myFunction1()" class="dropbtn">Select Software</button>'+
                    '<div id="myDropdown3" class="dropdown-content">'+
                        '<input type="text" placeholder="Search.." id="myInput" onkeyup="filterFunction()">'+
                        '<span<span<span style="display: block;" post-id='+post_id+' >List Rankings</span>'+
    '<span<span<span style="display: block;" post-id='+post_id+' >Best For</span>'+
    '<span<span<span style="display: block;" post-id='+post_id+' >Contender</span>'+        
                    '</div>'+
                    '</div>';
    
                              
                                const awards = document.querySelectorAll("#myDropdown3 span")
                                for (const button of awards) {
                          button.addEventListener('click', function(event) {
                           
                            var awardType = document.getElementById("selectAward").innerText = event.target.innerText;
                            post_id = event.target.getAttribute("post-id");
                            jQuery.ajax(
														{
															url: '<?php echo admin_url('admin-ajax.php') ?>',
