								...
								foreach($feature_all_ratings as $key => $fv)
								   {
									   $count=count($fv);
										...
										$features_ratings1['jsingh'] = '1';
										$features_ratings2['sunil'] = '2';
										foreach($features_ratings1 as $key => $values)
										{
											$readyForKeyComp[$key] = $key;
										}
										foreach($features_ratings2 as $key => $values)
										{
											$readyForKeyComp[$key] = $key;
										}
										$unmatchedKeys = array_diff_key($readyForKeyComp, $avg_feature);
										foreach($unmatchedKeys as $singleunmatchedKeys)
										{
											$feature_both_notexist[] = $singleunmatchedKeys;
										}
										print_r($feature_both_notexist);
										echo'cmooo';
										echo'<pre>';
										...
										//echo "feature";
										//echo $featureboth_loser;
										   

										//    if($compCount >= 2){
										//         if($votes<=5){echo $votes ;}
										//         else{echo "that being said $item is still voted well above the industry average." ;}
...
Looking at the top 3 features when it comes to <?php echo $feature_name1; ?> [{item b} wins the vote/ that being said {item a} is still voted well above the industry average.] Look at <?php echo $feature_name2; ?> both {item a} and {item b} scores below the industry. With regards to <?php echo $feature_name3; ?> neither solution has been recorded on our system has provided this feature</p>
