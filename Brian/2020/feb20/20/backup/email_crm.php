...
function getMessages($inbox, $debugFilesFolder, $filees)
{
    ...
    if ($lastUID != 1) 
	{
        ...
        while($lastMnoforHeader == 0 && $lastUIDforHeader >1 ){
            $lastMnoforHeader = imap_msgno($inbox, $lastUIDforHeader);
            $lastUIDforHeader--;
    ...
    file_put_contents($debugFilesFolder . "MainDebug.txt", "msgno for headerinfo is " . $lastMnoforHeader ."\n", FILE_APPEND);

    $header = imap_headerinfo($inbox, $lastMnoforHeader);
    file_put_contents($debugFilesFolder . "MainDebug.txt", "header is " . print_r($header,true) , FILE_APPEND);
    if($header === false)
	{
    ...
    if(strtotime($header->date) === false  && $header !== false)
	{ // eg. it fails for "Fri, 8 Mar 2019 03:50:23 -0700 (GMT-07:00)" acceptable example is "Fri, 08 Mar 2019 20:03:15 -0800"
    ...
    if (end($emailsObjects)->uid == ($lastUID - 1)) 
	{ //  removing the last mail which is repeatedly fetching if there are no new mails.
        $emailsObjects = array();
    }
    ...
    //$emails=Array();
    /*$normalUIDs = array(597,596,595,594,593,592,591,590,589,588);$trashUIDs = array(552,552,552,552,552,552,552,552,552,552,552);
    foreach($normalUIDs as $nu){
        $emails []= imap_msgno($inbox,$nu);  
    }
    $trashmails = array();
    foreach($trashUIDs as $tu){
        $trashmails []= imap_msgno($inbox,$tu);  
    }
    
    $emails = array_merge($emails,$trashmails);*/
    // end of new debugging system
	$messages = array();
	if ($emails) 
	{
        // $emails = $emails;
        ...
        foreach ($emails as $email_number) 
		{
			...
			//refine external css
           	...
           	$d = str_replace('rel="stylesheet"','rel="stylesheet" media="none" onload="if(media!=\'all\')media=\'all\'"',$d);
            $e = $messages->uid;
            ...
            if(in_array($email_number,$trashmails))
			{
                $trashed = "trashed_";
            }
			...
			{
                $myfile = file_put_contents($dir .$trashed. $mailname . ".txt", $saved_mail . PHP_EOL, FILE_APPEND);
            }
