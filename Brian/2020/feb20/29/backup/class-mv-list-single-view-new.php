...
$images = get_the_post_thumbnail_url($pid,array(150,150));?>
  ...
  if(empty($alternateinfo)){
   ...
   $item_result .= '<tr><th>Country</th><th>Usage</th>';
    ...
    array_push($popular_countries, 'and '.$last_element);

    $countries = implode(",",$popular_countries );
   ...
   foreach($comared_data as $post_idss)
    {
        ...
        $all_price_trim = str_replace("/mo","",$all_price); 
        $all_price_ids["_".$post_idss] = str_replace("$","",$all_price_trim);
    ...
    // $thirdarray= array(array(66, 51), array(52,90));
    ...
    // print_r($thirdarray);
        $result = array_merge_recursive($all_items_fs, $all_price_ids);  
        print_r($result);
...
//   foreach($all_items_fs as $key => $items_fs){
...
//   foreach($all_price_ids as $key => $price_ids){
...
//     if(array_keys($all_items_fs) == array_keys($all_price_ids))
//     {
// //         $abc = array_merge_recursive(array_values($all_items_fs), array_values($all_price_ids));
    ...
    <script>
     ...
     google.charts.load('current', {'packages':['scatter']});
      ...
      function drawChart () {
        ...
        data.addRows(
            <?php echo json_encode($thirdarray) ?>
