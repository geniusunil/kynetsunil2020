...
// var_dump($catobj );
    ...
    ?>
	    ...
	    <div class="single-list-data zf-item-vote" data-zf-post-id="<?php echo '136923'?>">
           ...
           <?php foreach($features_ratings as $f => $r){
              ...
              <?php wp_enqueue_script( 'axios', 'https://unpkg.com/axios/dist/axios.min.js' ); 
               ...
               var vm = new Vue({
                  ...
                  data: {
                    ...
                    foreach($features_ratings as $f => $r){ 
                        $f_ = str_replace(" ","_",$f);    
                        $label = $f;
                        $score = $r[total_score]/$r[votes];
                        echo "$f_ : {label : '$label', val : $score, color: 'green' }" ?>,
                  <?php } ?>
    ...
    //   alert('Hello ' + this.name + '!')
      // `event` is the native DOM event
        ...
        var feature_name = event.target.querySelectorAll('input')[0].getAttribute("data-obj");//(jQuery(event.target).find('input')[0]).getAttribute("data-obj");
...
axios.post( gMesLCData.ajaxurl, Qs.stringify( data ) )
	.then( response => {
	...
	.catch( error => console.log( error ) );
        ...
        jQuery.ajax({
            ...
            data: {post_id: zf_post_id, feature_name : feature_name, vote : vote, action: 'mes-lc-feature-rate'},
