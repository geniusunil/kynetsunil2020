...
function get_item_sidebar($post_id){
    ...
    $sidebar .= '<div class ="main_div_sidebar">';  
...
/************************************************************chart******************************************************************* */
   ...
   $item_result .= ' <div id="regions_div" style="width: 900px; height: 500px;"></div>';
...
// <!-- /************************************************************chart******************************************************************* */ -->
$review_id= get_the_ID();
$pricing_model_check = array();
$compObj = new Mv_List_Comparision();
$returns = $compObj->most_compared($review_id,20,true);

foreach ($returns as $compared) {
 
    $pricing_model_check = get_field( 'pricing_model',$compared );
    $item_coupon_all = get_post_meta($compared, 'coupons_list', true); //coupon     
    $pricing_starting_start[$compared] = get_field( 'price_starting_from',$compared );

    $reviews[] = get_overall_combined_rating($compared);   
   
    foreach($pricing_model_check as $price){
        $total_model[] =$price;
    } 
    foreach($item_coupon_all as $coupon_one ){
// print_r($key);
// print_r($coupon_one);
        $coupon_all_altr_post[] = $coupon_one;       
    }
    $free_trial_all = get_field( 'free_trial', $compared );
    if($free_trial_all == 1)
                {
                    $free +=$free_trial_all;
...
foreach($reviews as $review_one){
    $findrScore = $review_one['list'][overallrating][score]*10;
    $listrankord = $this->ranklist;
    $listrankord = $this->ranklist;
    foreach($listrankord as $listid => $rank){
        $noOflistitems = sizeof(get_field('list_items',$listid));
...
//  $free_trial_all_count = count($free_trial_all);

$coupon_filter = array_filter($coupon_all_altr_post); 
$coupon_all_list = count($coupon_filter );

$result = array_filter($pricing_starting_start);
$abc=array();  //for price and performance chart    
foreach($result as $key =>$price_start){
    // print_r($key);
    trim($price_start , "$");
//   $abc .= "[".$count++.",";
  $abc .= $price_start;
...
// echo "helllo";
// print_r($abc);
// print_r($findr_result);
// echo "end";
$abc11 = array(12,13,22,34 ,54 ,3,21, 2,2,3,5,6,7,8);
$findr_result99 = array(2,2,3,5,6,7,8,12,13,22,34 ,54 ,3,21) ;
// $all_list_main = array_combine($findr_result, $abc11);
// print_r($all_list_main);
...
$count_highest_model = array_count_values($total_model);  // pricing model 

foreach($count_highest_model as $count_model){
$price_list = $count_model;// price list for pricing model 
break;
}
$pricing_model = get_field( 'pricing_model', $post_id );
        $item_coupon = get_field( 'coupons_list', $post_id );        
        // print_r($item_coupon);
        $coupon_count = count($item_coupon);
 ...
 // <!-- /************************************************************chart******************************************************************* */ -->
       ...
       $item_result .= '<div id="circles" style="background:#2898f7;">
         <div class="viewport inview">
           ...
           $item_result .= '<circle r="44" cx="88" cy="88" fill="transparent"stroke="#bbb0b0" stroke-width="88" stroke-dasharray="calc('.$price_list.'  * 276.32 / 100) 276.32" transform="rotate(-90) translate(-176)" /><text class="detail-text" font-size="20" line-spacing="25" fill="#5BD49C"><tspan x="80" y="123">'.$price_list.'</tspan></text></g></g>';
           ...
           $item_result .= '<circle r="57" cx="62" cy="114" fill="transparent" stroke="#bbb0b0" stroke-width="114" stroke-dasharray="calc('.$coupon_all_list.' * 357.9 / 100) 357.9" transform="rotate(-90) translate(-176)" />';
           $item_result .= '<text class="detail-text" font-size="20" line-spacing="25" fill="#5BD49C"><tspan x="80" y="123">'.$coupon_all_list.'</tspan></text> </g><text font-size="20" line-spacing="25" fill="#FFFFFF"><tspan x="109.5" y="364">Free Trails</tspan></text>';
           $item_result .= ' <g transform="translate(0.000000, 377.000000)"><circle fill="#c2d2e0" stroke="#fff" stroke-width="2" cx="132" cy="132" r="132" class="circ-fill-circle" id="r16"></circle><circle r="66" cx="44" cy="132" fill="transparent" stroke="#bbb0b0" stroke-width="131" stroke-dasharray="calc('.$free.' * 414.48 / 100) 414.48" transform="rotate(-90) translate(-176)" /> ';
           $item_result .= ' <text class="detail-text" font-size="20" line-spacing="25" fill="#5BD49C"><tspan x="80" y="123">'.$free.'</tspan></text>';
...
//    $reviews = $rev->combined_review($post_id,'rwp_template_590f86153bc54');
  ...
  /************************************************************FOr NEW LAYOUT********************************************************* */     
    $title =  get_the_title($post_id).": Pricing and Features";
    $rating_item = do_shortcode( '[rwp_users_rating_stars id="-1"  template="rwp_template_590f86153bc54" size=20 post='.$post_id.']' );
...
$details .= '<div class="row">
      ...
      $details .= ''.$img.'</div> 
                            ...
                            <div >
                            <div class="item-subtitle"> FindrScore </div>
         ...
         $details .= '<div class="col-sm-6 item_list_content_sec">'; //item features and their progress bar first list                        
            $details .='<div class="item-description price_feature ">
                                    <div class="col-sm-6 easeofuse"> Accounting Features <br>
                                        <div class="progress"> 
                                            <div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:'.$accounting_list.'%">
                                    ...
                                    <div class="col-sm-6 easeofuse"> Ease of use <br>
                                        <div class="progress"> 
                                            <div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:'.$feature_list.'%">
                        ...
                        $details .='<div class="col-sm-6 easeofuse">Cost <br>
                                        <div class="progress"> 
                                            <div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:'.$price_item.'%">
                                            <span class="sr-only">'.$price_item.'</span>
                                             </div>
                                        </div>&nbsp;<span>'.$price_item.'/10<span>        
                                    </div>
                                    <div class="col-sm-6  easeofuse">Customer Service <br>
                                        <div class="progress"> 
                                            <div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:'.$customer_support.'%">
...
/*------------------------------------------------------section1 complte-----------------------------------------------------------*/
$details .= '<div class="row"><hr class="hr-line"> <div class="row"> 
