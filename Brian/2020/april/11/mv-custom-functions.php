		...
		?>
			...
			/* 	echo "sorted features";
				...
				foreach($this_top3_features as $feature=>$score){
					...
					$this_key_features = '<span>'.$f.'</span><v-slider
					v-on:click="greet"
					v-model="' . $f_ . '.val"
					:color="' . $f_ . '.color"
					:thumb-size="15"
					...
					max=10
					></v-slider>';
					$this_key_features .= '<span class="status">{{ ' . $f_ . '.status}}</span> <br>';
				}
			
					$arrToPass[$key]=array('Key Features'=>'',
							'Pros & Cons'=>'',
						'Pricing'=>'',
					'Rating'=>'');
					$arrToPass[$key]['Integrations']='';
