...
//funtion compare table in list page
    public function list_compare_table($post_id)
    {
...
jQuery.ajax({
							...
							type:'POST',
							data: {"action": "infinite_scroll", "page_no": <?php echo $current_page; ?>, "list_id": <?php echo $list_id; ?>, "sort": "<?php echo $_GET['sort']; ?>"},
							beforeSend: function() {
...
$html = ob_get_contents();
    ...
    public function get_item_sidebar($post_id)
    {
        ...
        $sidebar .= '<p class="visit_btn_sec"><a href="" class="visit_btn">' . $this->affiliate_button_text . ' <i class="fa fa-lock"></i> </a></p>';
...
// echo "2item rating feature";
           ...
           // foreach($item as $idd){
            //   echo $key;
                    ...
                    if($key <= 3){
                        ...
                        $key_feature_result .= '<div class="show" >'.$integarte_item_img;
                        $key_feature_result .= ' </div >';
                        $key_feature_result .= ' <div class="col-sm-6 integrate_name">';
                        $key_feature_result .= "<strong><span>" . $integarte_item_name . "</strong></span></div>";
                    } else{
                        $key_feature_result .= '<div class="col-sm-6 hide_li_1" > <div class="col-sm-5 integrate_img">';
                        $key_feature_result .= '<div class="" >'.$integarte_item_img;
                        $key_feature_result .= "<strong><span>" . $integarte_item_name . "</strong></span></div>";
                    ...
                    //     $key_feature_result .= '<div class="hide_li_1" >'.$integarte_item_img;
                    //     $key_feature_result .= "<strong><span>" . $integarte_item_name . "</strong></span></div>";
       ...
       </div><h5 style="font-weight:600; cursor: pointer;" id="seeMore_1">See More</h5>';
...
echo "all item id pricing chart end"; */
        ...
        // $items_names_3 = implode(",", $items_names);
        // $this->integrate_Avg = round(count($total_integrate_list) / count($all_item_id), 2);
        // if (!empty($all_integrated_list)) {
        //     $integrate_text = "Looking at the data collected on our platform, the average amount of integrations confirmed by similar solutions is $integrate_Avg.
        ...
        $pricing_result .= '<div class="col-sm-12" style="
        display: flex;
    "> ';
        $pricing_result .= '<div class="col-sm-6"> ';
        ...
        $pricing_result .= '<div class="col-sm-3" style="
        margin: auto;
    "> ';
...
$compObj = new Mv_List_Comparision();
    ...
    public function support_sec($post_id)
    {
        ...
        if ($all_support_score_avg < $item_score) {
            $support_result .= '<img src="' . esc_url(plugins_url('image/good-support.png', dirname(__FILE__))) . '">';

        } else {
            $support_result .= '<img src="' . esc_url(plugins_url('image/poor-support.png', dirname(__FILE__))) . '">';
        }
        $support_result .= '</div>';
        $support_result .= '<div class = "col-sm-4">';
 ...
 */        if($benchmark < $findrScore){
        ...
        $item_result .= '<div id="overview">
         ...
         We bought [19] of the [best Bluetooth earbuds] available in [2019], put them through a stringent series of examinations with the help of [total voters] like you, and assigned them FindrScores, all to help you make a smarter decision.  

Our algorithm looks at over [data points] such as user satisfaction, price, geographical quirks, how the must-have functionality like [feature 1], [feature 1] and [feature 3] compare against these [list number] solutions.

So whether you\'re looking for the cheapest, easiest to use, the best overall we ‘ve got you covered. 

Narrow down your search
Find the [list title - list number} for you, Tailor the data to find the {best video doorbell brands} for your needs. See which software offers the best [insert 3 top features] or which is the easiest to use.

		Filter by [Best Webinar Software (2019 Compared) in United Kingdom] – See what users from this country recommends
		Filter by Best Webinar Software (2019 Compared) in India – See what users from this country recommends
		Filter by free Webinar Software (2019 Compared) ] – See which is the best free solution.
		Filter by [cheapest Webinar Software (2019 Compared)] – See which is the most affordable solution.

		Looking for [category name] deals? – See today\'s best [category name] discount codes.
...
// $item_title = get_the_title($post_id);
        ...
        $conlast = '<div id="overview">
                        ...
                        <div>
                          <span itemprop="description"><b>Add excert here</b> ' . $contentP . '</span>
...
// echo "all total average score";
        ...
        $details .= '<div class="col-sm-12"> <div class="rating-count">' . $rating_item . '</div>
                                            ...
                                            ';
        $details .= $conlast;
        $details .= '<p class="visit_btn_sec"><a href="" class="visit_btn">' . $this->affiliate_button_text . '<i class="fa fa-lock"></i></a></p>';
        $details .= '</div> '; //div class="col-sm-6 item_list_image_sec

        $details .= '<div id="rating" class="col-sm-6 item_list_content_sec">'; //item features and their progress bar first list
