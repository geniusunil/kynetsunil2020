<?php
	...
	*/
	...
	function generate_list_html($postarr){
		$current_page = $paged = $_POST['page_no'];
	...
	/* 	if(isset($sorts) && $sorts =="free"){
			...
			$postarr = $this_list['affordable_price'];
		...
		?>
				...
				$videoItems = '';
				$compareItems = array();
				echo "<div id=\"app\"> <v-app id=\"inspire\">";
		...
		?>
					...
					//print_r($ranks);
					foreach ($ranks as $rid1 => $raval11)
					{
						if ($rid1 != $main_list_id111)
						{
							// echo '
							// 						<a class="ls_referal_link" data-parameter="listid" data-id="' . $main_list_id111 . '" href="' . get_permalink($rid1) . '"> <span class="rank-ts"><span class="rr-val">' . $raval11 . '</span> </span> <span class="badge-text"> in ' . get_the_title($rid1) . '</span></a>
							// 					';
							// break;
						}
					}
									...
									<div class="row" id="test"> 
										...
										<div class="<?php echo ($availability == 'no') ? 'item-not-available' : ''; ?>"></div>
											<div id="componentWrapper<?php echo $index ?>"  class="componentWrapper">
												...
												<input type="checkbox" id = "compre_data" value ="compare" name="compare" data-link="<?php echo $list_id; ?>" style="overflow:  hidden; position:  relative; top: 530px; left: 50px;">
...
>
  ...
  <v-tab-item>
    <v-card flat>
	  ...
	  <?php
		  ...
		  $item_rating_feature = get_field('features_list_ratings', $key);
	  /* 	echo "sorted features";
 ...
 ?>
  <v-tab-item>
    <v-card flat>
      <v-card-text>r2b <?php print_r($r2b); ?><br> rn2b <?php print_r($rn2b); ?></v-card-text>
    </v-card>
  </v-tab-item>  
</v-tabs>
    </v-card>
	...
	function generate_compare_link($posts = array())
	{
		...
		echo $url;
		// $resp = array("url" => $url, "post_id" => 148621);
		// echo json_encode($resp);
		
		
	}
	
	add_action('wp_ajax_generate_url', 'generate_compare_link'); // for logged in user
	add_action('wp_ajax_nopriv_generate_url', 'generate_compare_link'); // if user not logged in
	...
	function get_pros_cons($post_id){
        ...
        if ($this_price_starting_from) {
            $trimmedprice = trim(trim($this_price_starting_from, " "), "$");
        }
