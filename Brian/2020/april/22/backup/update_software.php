...
//		$html_before_form .= '<dt class= "review_title">' . $field['label'] . '</dt>';
        ...
        $html_before_form .= '<div >';        
            $html_before_form .= '<div class="coupons_container_qa post_edit_page"><span class="add_new_qa">Add Another Qa</span> <ul class="list_all qa_list">
 </ul> </div>';
            ...
            if(!empty($qustion_list) && is_array($qustion_list)){        
                ...
                foreach ($qustion_list as $qustion_lists){                   
                    if(!empty($qustion_lists)){                        
                        $html_before_form .= '<li><p> Question:  <input type="text" name="old_question[]" value="'.$qustion_lists[question].'" required></p>
...
jQuery(document).on('click','.add_new_qa',function () {
    ...
    jQuery(this).closest('.coupons_container_qa').find('ul').append('<li><label>QA</label><span class="remove_coupon">X cancel coupon</span></li> <li><label> Question : </label><input type="text" name="old_question[]"  value="" required> </li>'+
