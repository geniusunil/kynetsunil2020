	...
	*/
	...
	/* 	if(isset($sorts) && $sorts =="free"){
		...
		?>
					...
					$this_key_features = '<div class="features">';
...
$this_key_features.= '<ul class="featurespoints' . $addClass . '">';
foreach ($features as $ind => $feat)
{
	$this_key_features.= "<li>• $feat</li>";
if ($ind == 5)
{
break;
}
}
$this_key_features.= '</ul>';
if ($count > 6)
{
$rest = $count - 6;
$this_key_features.= '<ul class="otherlist"> <li class="others"><a class="mes-lc-li-link update_list_modified_link" data-zf-post-id="' . $list_id . '" data-zf-post-parent-id="' . $main_list_id . '" href="' . get_the_permalink($list_id) . '#features">+' . $rest . ' others </a></li></ul>';
}
}
$this_key_features.= '</div>';


				
					$arrToPass[$key]=array('key features'=>$this_key_features);
		?>
													</div>
												</div>
    ...
    <v-card>
      ...
      >
        <v-tab
          v-for="item in items<?php echo $key; ?>"
          :key="item.tab"
        >
          {{ item.tab }}
        </v-tab>
				...
				new Vue({
				  ...
				  data () {
					return {
					  tab: null,
					  ";
					  foreach($arrToPass as $id=>$items){
						echo "items".$id.":[";
						  foreach($items as $tabname=>$tabcontent){
							echo "{tab:'".$tabname."',content: '".$tabcontent."'},";
						  }
						  echo "],";
					  }
					  echo "}}
