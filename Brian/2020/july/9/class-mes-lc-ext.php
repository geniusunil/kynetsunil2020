...
if (!class_exists('Mes_Lc_Ext')){
	class Mes_Lc_Ext
	{
		...
		const VOTES_SINGLE_META_KEY = 'votes_given_singleitem';
		const COUPONS_META_KEY = 'coupons_list';
		const DOWNLOADS_LOCATION_META_KEY = 'downloads_in_';
		const VOTE_AJAX_ACTION = 'mes-lc-li-vote';
		
		//by jasbir 
		const VOTE_ITEM_AJAX_ACTION = 'mv-item-single-vote';
		const ITEM_VOTES_COOKIE_NAME = 'mes-lc-item-votes';
		//const SINGLE_ITEM_VOTE_TYPES = array(self::UP_VOTE_TYPE=> 1, self::DOWN_VOTE_TYPE=>-1);
		
		//vote types
		const VOTE_UP = 'up'; 											//1
		const VOTE_DOWN = 'down';										//2
		const VOTE_UNDO_UP = 'undo_up';									//3
		const VOTE_UNDO_DOWN = 'undo_down';								//4
		const VOTE_UNDO_UP_THEN_DOWN = 'undo_up_then_down';				//5
		const VOTE_UNDO_DOWN_THEN_UP = 'undo_down_then_up'; 			//6
		...
		function __construct()
		{
			...
			//new
			add_action( 'wp_ajax_' . self::VOTE_ITEM_AJAX_ACTION, array( $this, 'on_mv_list_single_vote' ) );
			add_action( 'wp_ajax_nopriv_' . self::VOTE_ITEM_AJAX_ACTION, array( $this, 'on_mv_list_single_vote' ) );
		...
		{
			if (isset($_POST['acf']) && isset($_POST['acf'][self::LIST_ITEMS_FIELD]))
			{
		...
		//by doc--------------------------------------------------------------------------
		function on_mv_list_single_vote()
		{
			$item_id = $_POST['post_id'];
			$vote_type = $_POST['vote_type'];
			$vote_type_final = $vote_type;
			$cookie_name = self::ITEM_VOTES_COOKIE_NAME;

			file_put_contents("meslcext.txt","cookie name: ".$cookie_name.PHP_EOL, FILE_APPEND);

			$cookie_value = $this->get_cookie_value($cookie_name);
			file_put_contents("meslcext.txt","cookie value: ".print_r($cookie_value,true).PHP_EOL, FILE_APPEND);
			
			if(!$this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_UP) && !$this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_DOWN))
			{
				file_put_contents("meslcext.txt","has not voted before ".PHP_EOL, FILE_APPEND);
				$vote_value = $vote_type;
			}

			else if (self::VOTE_UP == $vote_type)
			{
				file_put_contents("meslcext.txt","has voted before ".PHP_EOL, FILE_APPEND);
				if ($this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_UP))
				{
					$vote_value = self::VOTE_UNDO_UP;
					file_put_contents("meslcext.txt","cookie cookie_value before removal: ".print_r($cookie_value,true).PHP_EOL, FILE_APPEND);
					$cookie_value = $this->remove_voted_single_item($cookie_value, $item_id, self::VOTE_UP);
					file_put_contents("meslcext.txt","cookie cookie_value after removal: ".print_r($cookie_value,true).PHP_EOL, FILE_APPEND);
					$vote_type_final = false;
				}
				else if ($this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_DOWN))
				{
					$vote_value = self::VOTE_UNDO_DOWN_THEN_UP; 
					$vote_type_final = self::VOTE_UP;
					$cookie_value = $this->remove_voted_single_item($cookie_value, $item_id, self::VOTE_DOWN);
				}	
			}
			...
			{
				if ($this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_DOWN))
				{
					$vote_value = self::VOTE_UNDO_DOWN;
					$cookie_value = $this->remove_voted_single_item($cookie_value, $item_id, self::VOTE_DOWN);
					$vote_type_final = false;
				}
				else if ($this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_UP))
				{
					$vote_value = self::VOTE_UNDO_UP_THEN_DOWN; 
					$vote_type_final = self::VOTE_DOWN;
					$cookie_value = $this->remove_voted_single_item($cookie_value, $item_id, self::VOTE_UP);
				}
			}
			...
			if (isset($vote_value))
			{
				update_list_modified($item_id);
				file_put_contents("meslcext.txt","my vote = ". $vote_value .PHP_EOL ,FILE_APPEND);
				$total_votes = $this->update_vote($item_id, NULL, $vote_value);
				setcookie( $cookie_name, json_encode($this->set_voted_single_item($cookie_value, $item_id, $vote_type_final)),time() + self::ONE_DAY_IN_SECS, '/');
				$voted = 1;
			}

			$cookie_value = $this->get_cookie_value($cookie_name);
			echo json_encode($total_votes);
			wp_die();
		}
		...
		function on_list_item_vote()
		{
			...
			if (array_key_exists($vote_type,self::VOTE_TYPES))
			{
				...
				{
					if(!$this->has_voted($cookie_value, $item_id, self::UP_VOTE_TYPE) && !$this->has_voted($cookie_value, $item_id, self::DOWN_VOTE_TYPE))
					{
						$vote_value = self::VOTE_TYPES[$vote_type];
					}
		...
		function has_voted($data, $item_id, $vote_type)
		{
			...
			{
				$arr = $data[$vote_type];
				if (is_array($arr) && in_array($item_id,$arr))
				{
					return true;
				}
		 	}	
		...
		function has_voted_single_item_vote($cookie_array, $post_id, $vote_type)
		{
			if (is_array($cookie_array))
			{
				//$arr = $cookie_array[$vote_type];
				foreach($cookie_array as $key => $single_cookie_array){
					if($key == $post_id)
					{
						if($single_cookie_array == $vote_type)
							return true;
					}
				}
		...
		function set_voted_single_item($data, $item_id, $vote_type)
		{

			if (!is_array($data)) $data = array();	
			
			// if (!isset($data[$item_id][$vote_type])) $data[$item_id][$vote_type] = 0;
		
			// if (!is_array($data[$vote_type])) $data[$vote_type] = array();

			if ($vote_type )
			{
				$data[$item_id] = $vote_type;
			}
			file_put_contents("set_voteitem.txt","after code".print_r($data,true),FILE_APPEND);
		...
		{
			if (isset($o_vote_type) && isset($data[$o_vote_type]) && is_array($data[$o_vote_type]) && in_array($item_id, $data[$o_vote_type]))
			{
		...
		function remove_voted_single_item($data, $item_id, $o_vote_type)
		{
			$responseArr = array();
			if(isset($o_vote_type))
			{
		...
		function update_vote($post_id, $post_parent_id, $vote)
		{
			...
			{
				$total_votes = get_post_meta( $post_id, self::VOTES_SINGLE_META_KEY, true );
				file_put_contents("meslcext.txt","inside update ovte single meta key ".print_r($total_votes,true),FILE_APPEND);

				if (!is_array($total_votes))
				{
					$total_votes = array();
				}
				if(!isset($total_votes['up']))
				{
					$total_votes['up'] = 0;
				}
				if(!isset($total_votes['down']))
				{
					$total_votes['down'] = 0;
				}
				...
				if($vote == 'up')
				{
					$total_votes['up'] = $total_votes['up'] + 1;
				}
				elseif($vote == 'down')
				{
					$total_votes['down'] = $total_votes['down'] + 1;
				}	
				elseif($vote == 'undo_up') 
				{
					$total_votes['up'] = $total_votes['up'] - 1;
				}
				elseif($vote == 'undo_down')
				{
					$total_votes['down'] = $total_votes['down'] - 1;
				}
				elseif($vote == 'undo_up_then_down')
				{
					$total_votes['up'] = $total_votes['up'] - 1;
					$total_votes['down'] = $total_votes['down'] + 1;
				}
				elseif($vote == 'undo_down_then_up')
				{
					$total_votes['down'] = $total_votes['down'] - 1;
					$total_votes['up'] = $total_votes['up'] + 1;
				}

				file_put_contents("meslcext.txt","after  update ovte single meta key ".print_r($total_votes,true),FILE_APPEND);
				update_post_meta( $post_id, self::VOTES_SINGLE_META_KEY, $total_votes ); //
				return $total_votes;
			}
			else
			{
	...
	if($vote <0){
		$increment = -1;
			...
			if($featureListratings[$feature_name]['votes'] < 0){
				$featureListratings[$feature_name]['votes']=0;
				$featureListratings[$feature_name]['average']=0;
			}
			
			if($featureListratings[$feature_name]['total_score'] < 0){
				$featureListratings[$feature_name]['total_score']=0;
				$featureListratings[$feature_name]['average']=0;
			}
			file_put_contents("meslcext.txt","cur_feature_score : ".print_r($featureListratings,true),FILE_APPEND);
