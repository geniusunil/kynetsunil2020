<?php
	...
	*/
	...
	function my_item_rank($pId,$main_list_id)
	{
		...
		if (!empty($lists) && is_array($lists))
		{
			foreach ($lists as $id)
			{
				if ($id!=$main_list_id && is_string(get_post_status($id)))
				{
					$rank = get_item_rank($id, $itemiid);
					$listrankord[$id] = $rank;
					break;
	...
	/* 	echo "postarr";
		...
		break;
			...
			// echo "items per page $items_per_page current page $current_page";
		   	if($ajax===false){
				$static_val = array_slice( $postarr, $items_per_page*($current_page-1) , $items_per_page , true);
		...
		?>
											...
											<div class="productbox zf-list_item"><?php if ($availability == 'no')
					{
						echo "<div class='pos-relative'><div class='text-overlay'></div>";
													...
													<?php // if($list_id == $maxsScorePost){
					// $ranks = my_item_rank($list_id,$main_list_id);
					//print_r($ranks);
...
>
  ...
  $this_who = get_field('best_for', $key, false);
...
<?php }
 if (!empty($this_who)) {
	...
	$who_result = '<div class = "item-integrateswell-with item-sec-div">';
	$who_result .= '<h3> Integrates well with  </h3>';

	 
	 $who_result .= '<div class="col-sm-12">'.$integrate_text.'</div>';


	// print_r($this_integrations);
	// echo"industery end";
	$who_result .= '<div class="roiiw"><div class="col-sm-12 more_data">';
   ...
   // foreach($item as $idd){
	//   echo $key;
			...
			if($key <= 3){
				$who_result .= '<div class="col-sm-6" ><a href="'.get_the_permalink($idd).'"> <div class="integrate_img">';
				$who_result .= '<div class="show" >'.$integarte_item_img;
				$who_result .= ' </div >';
				$who_result .= ' <div class="integrate_name">';
				$who_result .= "<strong><span>" . $integarte_item_name . "</strong></span></div></a>";
			} else{
				$who_result .= '<div class="col-sm-6 hide_li_1" ><a href="'.get_the_permalink($idd).'"> <div class="integrate_img">';
				$who_result .= '<div class="" >'.$integarte_item_img;
				$who_result .= "<strong><span>" . $integarte_item_name . "</strong></span></div></a>";
			...
			//     $who_result .= '<div class="hide_li_1" >'.$integarte_item_img;
			//     $who_result .= "<strong><span>" . $integarte_item_name . "</strong></span></div>";
	...
	$who_result .= '</div>'; //item-integrateswell with"
...
<?php }?>
	...
	add_action('wp_ajax_awards_by_post_id', 'get_awards_by_post_id'); // for logged in user
	add_action('wp_ajax_nopriv_awards_by_post_id', 'get_awards_by_post_id'); // if user not logged in
	
	add_action('wp_ajax_reviews_by_post_id', 'get_reviews_by_post_id'); // for logged in user
	add_action('wp_ajax_nopriv_reviews_by_post_id', 'get_reviews_by_post_id'); // if user not logged in
	
	function get_awards_by_post_id(){
		$post_id=intval($_POST['post_id']);
		
		// echo $award_type;
		// echo $post_id;
		// if($award_type == "list-ranking"){
		// list rankings
		$lists = get_item_ranks($post_id);
		// file_put_contents("mvcf.txt","inside get_lists_by_post_id ".PHP_EOL.print_r($lists,true).PHP_EOL,FILE_APPEND);

		// echo json_encode($lists);
		$permalink = get_the_permalink($post_id);
		$awards = array('list_rankings'=>array(),
						'best_for'=>array(),
						'market_radar'=>array(),
					'permalink'=>$permalink);
		$tops = array(5,10,20,30,40,50);
		$i=0;
		foreach($lists as $listID=>$rank){//get_field('add_to_list',$post_id);
			$key = $i." ";
			
			if($rank<=50){
				$awards['list_rankings'][$key] = array();
				$awards['list_rankings'][$key]['id']=$listID;
				$awards['list_rankings'][$key]['permalink']=get_the_permalink($listID);
				if($rank>0 && $rank<=3){
					$awards['list_rankings'][$key]['ranking_quota']="Voted #".$rank;
				}else{
					foreach($tops as $top){
						if($rank<=$top){
							$awards['list_rankings'][$key]['ranking_quota']="Top ".$top;
							break;
						}
					}
				}
				$list_singular_title = get_field('list_content_title_singular', $listID, true);
				if($list_singular_title == ""){
					$list_singular_title = get_the_title($listID);
				}
				
				$awards['list_rankings'][$key]['singular_title']=$list_singular_title;
			}
		...
		foreach ($all_item_id as $all_items) {
            ...
            if(isset($rat['list']['overallrating']['score'])){
                ...
                $overallindustryratings[] = $rat['list']['overallrating']['score'];
            }
        ...
        $this_customer_support = $reviews['list']['customersupport']['score'];
		$this_features_functionality = $reviews['list']['featuresfunctionality']['score'];
		$item_rat = $reviews['list']['overallrating']['score'];
		$customersupport_count = array_sum($customersupport);
		$customersupport_feature_average = round($customersupport_count / count($all_item_id) , 2);
		file_put_contents("mvcf.txt","inside get_awards_by_post_id ".PHP_EOL."thiscustomersupport $this_customer_support".PHP_EOL."customersupport_feature_average $customersupport_feature_average",FILE_APPEND);

		if ($this_customer_support > $customersupport_feature_average) {
            $awards['best_for'][] = "Customer support";
		}

		$ease_count = array_sum($easeofuse);
        $ease_average = round($ease_count / count($all_item_id) , 2); //ease average
		if ($this_ease_of_use > $ease_average) {
            $awards['best_for'][] = "Ease of use";
		}
		$over_high_score = max($overallindustryratings);
		file_put_contents("mvcf.txt","item_rat $item_rat".PHP_EOL."over_high_score ".$over_high_score,FILE_APPEND);

		if ($item_rat >= $over_high_score) {
            $awards['best_for'][] = "Complete solution ";
        }

		$this_item_price_start = get_field('price_starting_from', $post_id);

		$this_item_price = (float) filter_var($this_item_price_start, FILTER_SANITIZE_NUMBER_FLOAT); //item price
		$this_pricing_plan = get_field('plan', $post_id);
		$item_pricing_model = get_field('pricing_model', $post_id);
		$all_price_avg = 0;
		if(in_array('freemium', $item_pricing_model) || in_array('open_source', $item_pricing_model)){
			$awards['best_for'][] = "Low cost software";
		}else{
        ...
        if ($this_pricing_model == "subscription" || $this_pricing_model == "one_time_license") {
			...
			// echo "price sum".$price_sum;
			file_put_contents("mvcf.txt","price sum $price_sum".PHP_EOL."count price start ".count($price_start),FILE_APPEND);

            $all_price_avg = $price_sum / count($price_start); // all items avg price in a sorted way

		}
		file_put_contents("mvcf.txt","inside get_awards_by_post_id ".PHP_EOL."this_pricing_model $this_pricing_model".PHP_EOL."this_item_price $this_item_price".PHP_EOL."all_price_avg $all_price_avg",FILE_APPEND);
		...
		$item_feature_rating = get_or_create_feature_ratings($post_id); //get_field( 'features_list_ratings', $item_id );
        // echo "item rat score";
		...
		// $awards['best_for'][]=$mutual_feature_names;
		//end best for
        ...
        foreach ($returns as $single_compared) {
            // $radar_data[$single_compared] = array( calc_frequency($single_compared), get_or_calc_fs_individual($single_compared));
			// $find_avg_findscore[$single_compared] = get_or_calc_fs_individual($single_compared);
			$this_RF=calc_frequency($single_compared);
			$this_FS=get_or_calc_fs_individual($single_compared);
			if($post_id == $single_compared){
				$currentFS = $this_FS;
				$currentRF = $this_RF;
			}
			if(isset($minFS)){
		...
		$minRF-=10;
		$maxRF+=10;
		$rfProportion=($maxRF-$minRF)/12;
		$fsProportion=($maxFS-$minFS)/12;
		$awards['market_radar']=array();
		$currentFSby12= $currentFS/$fsProportion;
		$currentRFby12= $currentRF/$rfProportion;
	
		if($currentRFby12>=4 && $currentFSby12>=4){
			if($currentRFby12<=9){
				$y=36-(3*$currentRFby12);
			}else{
				$y=(36-$currentRFby12)/3;
			}
			
			
			// $y=$y1>$y2?$y1:$y2;
			// $awards['market_radar']['yreqleader']=$y;
			// $awards['market_radar'][]=$currentFSby12;
			if($currentFSby12>=$y){
				$awards['market_radar'][]="Leader";
			}
			else{
				if($currentRFby12<=6){
					$y=24-(3*$currentRFby12);
				}else{
					$y=(24-$currentRFby12)/3;
				}
				
				// $awards['market_radar']['yreqstrongperf']=$y;

				// $y=$y1>$y2?$y1:$y2;
				if($currentFSby12>=$y){
					$awards['market_radar'][]="Strong Performer";
				}
			}
		...
		// check leader
		// equation is 3x+y-36 = 0; and x+3y-36=0;
	...
	function get_reviews_by_post_id(){
		$post_id=intval($_POST['post_id']);
		
		$rating = get_overall_combined_rating($post_id);
		$permalink = get_the_permalink($post_id);
		$overallScore = $rating['list']['overallrating']['score'];
		$ratingscount = $rating['count'];
		$reviews=array('overallscore'=>$overallScore,'ratingscount'=>$ratingscount,'permalink'=>$permalink);
	...
	add_action('wp_ajax_store_get_link', 'store_image_get_link'); // for logged in user
	add_action('wp_ajax_nopriv_store_get_link', 'store_image_get_link'); // if user not logged in
	function store_image_get_link(){
		$data = array();
		$randomString = generate_string();
		$filename = $_POST['filename'].$randomString.".png";
		// echo content_url('badges');
		
		// $result=file_put_contents('../wp-content/badges/'.$filename,$_POST['image']);
		// header( "Content-type: image/png" );
		
		$datai = substr($_POST['image'], strpos($_POST['image'], ",") + 1);
		$decodedData = base64_decode($datai);
		$fp = fopen('../wp-content/badges/'.$filename, 'wb');
		fwrite($fp, $decodedData);
		fclose($fp);
		// echo "/canvas.png";

		// $data['filewritten']=$result;
		// $data['filenamereceived']=$_POST['filename'];
		// $data['imagereceived']=$_POST['image'];
		$data['link']=content_url('badges').'/'.$filename;
		echo json_encode($data);
		wp_die();
	}

	function store_image_get_link_test(){
		$data = array();
		$randomString = generate_string();
		$filename = 'dummy'.$randomString.".png";
		// echo content_url('badges');
		if (!file_exists('wp-content/badges/')) {
			mkdir('wp-content/badges/', 0777, true);
		}
		file_put_contents('wp-content/badges/'.$filename,"hello");
		$data['link']=content_url('badges').'/'.$filename;
		echo json_encode($data);
		// wp_die();
	}
	add_action('wp_ajax_send_feature_vote', 'send_feature_vote'); // for logged in user
	add_action('wp_ajax_nopriv_send_feature_vote', 'send_feature_vote'); // if user not logged in
	function send_feature_vote(){
		$data=array();
		$post_id= $_POST['post_id'];
		$feature_name = $_POST['feature_name'];
		$vote = $_POST['vote'];
		$alreadyChecked = get_post_meta($post_id,'already_checked_no_feature',true);
		$features = get_post_meta($post_id,'features_list',true);
		$new_features=get_post_meta($post_id, 'new_features', true);
		if($new_features == null){
			$new_features = array();
		}
		if(isset($new_features[$feature_name])){
			$new_features[$feature_name][$vote]+=1;
			
		}else{
			if($vote=='yes'){
				$new_features[$feature_name]['yes']=1;
				$new_features[$feature_name]['no']=0;
			}
			else{
				$new_features[$feature_name]['no']=1;
				$new_features[$feature_name]['yes']=0;
			}
				
		}
		if($new_features[$feature_name][$vote] > 2 ){
			if($vote=='yes'){
				...
				if(!in_array($feature_name,$features)){
					...
					if($features_ratings === null ){
						$features_ratings = array();
					}
					if(!isset($features_ratings[$feature_name])){
						$features_ratings[$feature_name]= array(
							'total_score' => 0,
							'votes' => 0,
							'average' => 0
						);
						update_post_meta($post_id, 'features_list_ratings', $features_ratings);

					}
					
					update_post_meta($post_id, 'features_list', $features);
				}
				...
				if(!in_array($feature_name,$alreadyChecked)){
					$alreadyChecked[]=$feature_name;
					update_post_meta($post_id, 'already_checked_no_feature', $alreadyChecked);
				}
				
			}	
			unset($new_features[$feature_name]);
		}

		update_post_meta($post_id, 'new_features', $new_features);
		$data['new_features']=$new_features;
		$data['features']=$features;
		$data['rejected-features']=$alreadyChecked;
		echo json_encode($data);
		wp_die();
	}
	add_action('wp_ajax_send_quit_vote', 'send_quit_vote'); // for logged in user
	add_action('wp_ajax_nopriv_send_quit_vote', 'send_quit_vote'); // if user not logged in
	function send_quit_vote(){
		$post_id= $_POST['post_id'];
		$voteto = $_POST['voteto'];
		$data=array();
		
		$quitVotes = get_post_meta($post_id, 'quit_votes', true);
		if($quitVotes == null){
			$quitVotes = array();
		}
		if(!isset($quitVotes[$voteto])){
			$quitVotes[$voteto]=0;
		}
		$quitVotes[$voteto]++;
		update_post_meta($post_id, 'quit_votes', $quitVotes);
		$data=$quitVotes;
		echo json_encode($data);
	...
	add_action('save_post', 'scrape_reviews');
	function scrape_reviews($post_id){
		$links= get_field('import_other_sites',$post_id);
		// print_r($links);
		file_put_contents("mvcf.txt",print_r($links,true).PHP_EOL,FILE_APPEND);
	...
	{
		$features_ratings = get_field('features_list_ratings', $post_id);
		// echo "inside get or create feature ratings";
	...
	function calc_industry_averages($item_id,$industry_items){
        $averages = array();
        foreach ($industry_items as $ii) 
		{
            $rat = get_overall_combined_rating($ii); //Overall combined rating
            if(isset($rat['list']['easeofuse']['score'])){
                $easeofuse[] = $rat['list']['easeofuse']['score'];
            }
            if(isset($rat['list']['easeofuse']['score'])){
            ...
            $price = get_field('price_starting_from', $ii);
            if(isset($rat['list']['overallrating']['score'])){

                $overallindustryratings[] = $rat['list']['overallrating']['score'];
            }
            $price_trim[] = (float) filter_var($price, FILTER_SANITIZE_NUMBER_FLOAT);//str_replace("$", "", $price);
            $all_features_ratings[$ii] = get_field('features_list_ratings', $ii);
            $all_item_findrscore[$ii] = get_or_calc_fs_individual($ii);
            $all_pricing_model[$ii] = get_field('pricing_model', $ii);
		...
		// echo "raw item price".$this_item_price_start;
		$this_item_price = (float) filter_var($this_item_price_start, FILTER_SANITIZE_NUMBER_FLOAT,FILTER_FLAG_ALLOW_FRACTION); //item price
            ...
            foreach ($all_pricing_model as $key => $pricng_model) {
                ...
                if (!in_array($this_pricing_model, $pricng_model)) {
                    ...
                    if ($this_pricing_model == 'subscription') {
                        ...
                        echo "price<br>"; */
                        // print_r( $price);
						$int = (float) filter_var($price, FILTER_SANITIZE_NUMBER_FLOAT,FILTER_FLAG_ALLOW_FRACTION);
        ...
        $averages['price']=$all_price_avg;
        $over_high_score = max($overallindustryratings); //overall highest score
        $averages['bestoverallscore']=$over_high_score;
        arsort($all_item_findrscore);
        $all_fs_sum = array_sum($all_item_findrscore);
        $fsavg =  $all_fs_sum / count($industry_items);
        $averages['findrScore']=$fsavg;
        ...
        if(is_array($all_features_ratings))
		{
            ...
            $averages['features']=$total_score_feature_ind_avg;
        /*     arsort($total_score_feature_ind_avg);
            if(!empty($item_feature_individual_avg))
            {
                foreach ($item_feature_individual_avg as $key => $feature_one) {
            ...
            $ease_average = round($ease_count / count($industry_items) , 2); //ease average
            $averages['easeofuse']=$ease_average;

            $functionality_count = array_sum($functionality_feature);
            $functionality_feature_average = round($functionality_count / count($industry_items) , 2); // feature functionality average
            $averages['functionality']=$functionality_feature_average;

            $valueformoney_count = array_sum($value_money);
            $valueformoney_feature_average = round($valueformoney_count / count($industry_items) , 2); // valueformoney average
            $averages['valueformoney']=$valueformoney_feature_average;
            $customersupport_count = array_sum($customersupport);

            // echo "customersupport_count";
            // print_r($customersupport_count);

            $customersupport_feature_average = round($customersupport_count / count($industry_items) , 2); // customersupport  average
            $averages['customersupport']=$customersupport_feature_average;
	...
	function get_or_refresh_best_for($post_id,$industry_averages,$refresh=false){
		$best_for=array();
	/* 	echo "<pre>industry averages";
		print_r($industry_averages);
		
		echo "this feature ratings";
		print_r($this_features_ratings);
		echo "</pre>"; */
		$this_features_ratings= get_or_create_feature_ratings($post_id);
		if(!empty($industry_averages['features']))
		{
			$item_feature_individual_avg = $industry_averages['features'];
            foreach ($item_feature_individual_avg as $key => $feature_one) {
				$this_score = 0;
				if(isset($this_features_ratings[$key])){
					$this_score = $this_features_ratings[$key]['average'];
				}
				/* echo "feature_one ".$feature_one;
				echo "this_score ".$this_score; */
                if ($feature_one < $this_score) {

					$mutual_feature_name = $key;
					$best_for []= $mutual_feature_name;
                }
      ...
      /*   echo "get overall combined rating";
        ...
        $findrScore = $reviews['list']['featuresfunctionality']['score'] ;
        $this_ease_of_use += $reviews['list']['easeofuse']['score'] ;
        $this_customer_support= $reviews['list']['customersupport']['score'];
		$findrScore = $reviews['list']['valueformoney']['score'] ;
		$item_rat = $reviews['list']['overallrating']['score'];

        if ($this_customer_support > $industry_averages['customersupport']) {
			$bestForCustSupport = "Customer support";
			$best_for []= $bestForCustSupport;
        }
        // echo "ease of use this item score ".$this_ease_of_use;
		// echo "ease_average ".$ease_average;
	
        if ($this_ease_of_use > $industry_averages['easeofuse']) {
			$ease_dis = "Ease of use";
			$best_for []= $ease_dis;
        }
		
        if (($this_item_price < $industry_averages['price'] ) || $this_pricing_model == 'freemium' || $this_pricing_model == 'open_source') {
        ...
        if ($item_rat >= $industry_averages['bestoverallscore']) {
			$overlist = "Complete solution ";
			$best_for []= $overlist;
        }
		
        if (empty($bestForCustSupport) && empty($ease_dis) && empty($price_item) && empty($overlist) && empty($mutual_feature_name)) {
	...
	function decide_filters($post_id,$post_id2=null){
		$compObj = new Mv_List_Comparision(null,true);
		$compared = $compObj->most_compared($post_id, 9, true);
		...
		$filters = array();
		$lists=get_field('add_to_list',$post_id);
		$refLists = array();
		foreach($lists as $list){
			$refLists[]=$list->ID;
		}
		// echo "post id ".$post_id;
		// echo "post id2 ".$post_id2;
		$refLists2 = array();
		if($post_id2!=null){
			// echo "inside";
			
			$lists2=get_field('add_to_list',$post_id2);
		...
		$refListsFinal = array_unique(array_merge($refLists,$refLists2));
		/* echo "reflistsfinal ";
		print_r($refListsFinal); */
		$rflVotes = array();
		foreach($refListsFinal as $rlf){
			$finalVotes = do_shortcode("[total_votes id=$rlf]");
			$rflVotes [$rlf]= $finalVotes;
		}
		arsort($rflVotes);
		$rflVotes5 = array_slice($rflVotes,0,5,true);
		/* echo "rflvotes5";
		print_r($rflVotes5); */
		foreach($rflVotes5 as $key=> $list5){
			$items = get_field('list_items',$key);
		...
		$filters['most_compared']=$compared;
	
		
		return $filters;
	}
	function generate_radar_chart($post_id,$key,$arr){
								// $class= "hide_radar";
								// if($key=='most_compared'){
								...
								$doct = getClosest($find_average, $find_avg_findscore);
								?>
								<div id="radarChart<?php echo $key; ?>";>
								<div class="background_softwareFinderRadar" style="width: 70%;height: 500px;margin: 0 auto;">
											<div id="softwareFinderRadar<?php echo $key; ?>" ></div>
										</div>
										<div class="col-md-12">
										<p>
										...
										<script type="text/javascript">
											...
											google.charts.setOnLoadCallback(drawSeriesChart_<?php echo $key; ?>);

											function drawSeriesChart_<?php echo $key; ?>()
											{
												...
												var chart = new google.visualization.BubbleChart(document.getElementById('softwareFinderRadar<?php echo $key; ?>'));
												google.visualization.events.addListener(chart, "ready", addBG<?php echo $key; ?>);

												chart.draw(data, options);
											}
											function addBG<?php echo $key; ?>()
											{
												// console.log("all done");
												// var svg = document.querySelector(".background_softwareFinderRadar svg");
												// console.log({svg});
												var attsRect = document.querySelector(".background_softwareFinderRadar<?php echo $key; ?> svg").children[1].children[0];
												console.log({attsRect});
												var parentElement = document.querySelector(".background_softwareFinderRadar<?php echo $key; ?> svg").children[1];
												console.log("here");
												...
												<?php if($key!='most_compared'){ ?>
													jQuery('#radarChart<?php echo $key; ?>').addClass('hide_radar');
												<?php } ?>
	...
	function get_pros_cons($post_id,$total_score_feature_ind_avg=null,$best_for=null){
		// echo "best for prs cons";
		// print_r($best_for);
		$r2b=array();
		$rn2b=array();
        ...
        $compObj = new Mv_List_Comparision();
        $lists = $compObj->most_compared($post_id, 16, true);

        $max = 0;

        foreach ($lists as $alternate) {

            $price = get_field('price_starting_from', $alternate);
			$price = (float) filter_var($price, FILTER_SANITIZE_NUMBER_FLOAT);
            if ($price > $max) {
                $max = $price;
	...
	/* 	echo "avgnof ".$avgnof." ";
		...
		$ffound=0;
		foreach($features_ratings as $f=>$r){
			if($r['average'] > $total_score_feature_ind_avg[$f]){
				$percent = round(($r['average']*100/$total_score_feature_ind_avg[$f])-100);
				$r2b[]=$f ." feature has been rated $percent% above industry average";
				$ffound++;
			}
			if($ffound>2){
				break;
		...
		foreach($features_ratings as $f=>$r){
			if($r['average'] < $total_score_feature_ind_avg[$f]){
				$percent = round(100-($r['average']*100/$total_score_feature_ind_avg[$f]));
				$rn2b[]=$f ." feature has been rated $percent% below industry average";
				$ffound++;
			}
			if($ffound>2){
				break;
		...
		$countLeaders=0;
		$seeMoreAppend = "";
		$seeMoreAppend2 = "<br><a href='#awards'>See full list</a>";
        foreach ($ranklist as $listid => $rank) {
			$list_plural_title=get_list_title_acf($listid,'plural');
            if ($rank < 4) {
				if($countLeaders>=2){
					$seeMoreAppend = $seeMoreAppend2;
				}
				$r2b[] = "Category leader in " . $list_plural_title.$seeMoreAppend;
				$countLeaders++;
            } elseif ($rank < 11) {
				if($countLeaders>=2){
					$seeMoreAppend = $seeMoreAppend2;
				}
                $r2b[] = "A contender in " . $list_plural_title.$seeMoreAppend;
				$countLeaders++;
			}
			// echo "countleaders ".$countLeaders;
			if($countLeaders>2){
				break;
			}
        }
		...
		foreach($best_for as $bf){
			if(trim($bf)!='')
				$r2b[]="Best for ".$bf;
		}
		return array('r2b'=>$r2b,'rn2b'=>$rn2b);
	}
	function has_feature_question($post_id,$industry_items){
		$i=0;
		$html="";
		$item_feature_lists = array();
		$currentFeatures = get_field('features_list', $post_id);
		$alreadyChecked = get_post_meta($post_id,'already_checked_no_feature',true);
		if($alreadyChecked==null){
			$alreadyChecked=array();
		}
		$html.= "<div style='display:flex;' class='feature_vote'>";
		$found=false;
		foreach($industry_items as $key=>$id){
			...
			foreach($this_item_features as $sfl){
				if(!in_array($sfl,$alreadyChecked) && !in_array($sfl,$currentFeatures)){
					
					$html.= "<span style='margin:auto 0;'>Does this item has ".$sfl." feature?</span> <button class='feature_ask' onclick='send_feature_vote(\"yes\",\"$sfl\",\"$post_id\",this)'>yes</button><button class='feature_ask' onclick='send_feature_vote(\"no\",\"$sfl\",\"$post_id\",this)'>no</button>";
					$found=true;
					break;
				}
...
function get_list_title_acf($list_id,$sop='plural'){
	$title = get_field('list_content_title_'.$sop, $list_id, true);
	if($title == ""){
