...
//          'field' => 'slug',
	...
	function kv_handle_attachment($file_handler,$post_id) { //http://www.kvcodes.com/2013/12/create-front-end-multiple-file-upload-wordpress/
	...
	function my_save_post( $post_id ) {
		...
		if ( $_FILES ) { 
			...
			foreach ($files['name'] as $key => $value) { 			
				...
				$files = $_FILES["kv_multiple_attachment_thumbs"];  
				foreach ($files['name'] as $key => $value) { 			
						if ($files['name'][$key]) { 
							$file = array( 
								'name' => $files['name'][$key],
