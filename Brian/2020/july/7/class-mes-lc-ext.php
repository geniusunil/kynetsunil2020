...
if (!class_exists('Mes_Lc_Ext')){
	class Mes_Lc_Ext
	{
		...
		function on_mv_list_single_vote()
		{
			...
			file_put_contents("meslcext.txt","cookie cookie_value: ".print_r($cookie_value,true).PHP_EOL,FILE_APPEND);
			...
			else if (self::VOTE_UP == $vote_type)
			{
				...
				if ($this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_UP))
				{
					...
					$cookie_value = $this->remove_voted($cookie_value, $item_id, self::VOTE_UP);
				...
				else if ($this->has_voted_single_item_vote($cookie_value, $item_id, self::VOTE_DOWN))
				{
					...
					$cookie_value = $this->remove_voted($cookie_value, $item_id, self::VOTE_DOWN);
				}	
			}
			...
			if (isset($vote_value))
			{
				...
				$votes_count = $this->update_vote($item_id, NULL, $vote_value);
		...
		function has_voted_single_item_vote($data, $item_id, $vote_type)
		{
		 	if ($data && is_array($data) && isset($data[$item_id][$vote_type])){
				return true;
		 	}	
			return false;
		}
		...
		function set_voted_single_item($data, $item_id, $vote_type)
		{
			...
			if ($vote_type ){
				if(isset($data[$item_id][$vote_type])){
					$data[$item_id][$vote_type]++;
				}
					
				else{
						$data[$item_id][$vote_type]=1;
		...
		function update_vote($post_id, $post_parent_id, $vote)
		{
			...
			if($post_parent_id == NULL)
			{
				...
				$total_votes[$post_id] = round((float)$total_votes[$post_id] + (float)$vote, 2);
				update_post_meta( $post_id, self::VOTES_META_KEY, $total_votes );
				return $total_votes[$post_id];
			}
			else
			{
