...
if (!class_exists('Mes_Lc_Ext')){
	class Mes_Lc_Ext
	{
		...
		const ITEM_VOTES_COOKIE_NAME = 'mes-lc-item-votes_';
		...
		{
			//file_put_contents("meslcext.txt"," in on_mv_list_single_vote",FILE_APPEND);

			$item_id = $_POST['post_id'];
			$vote_type = $_POST['vote_type'];
			
			$cookie_name = self::ITEM_VOTES_COOKIE_NAME . $item_id;
			...
			{
				$vote_value = self::SINGLE_ITEM_VOTE_TYPES[$vote_type];
			}
			else if (self::UP_VOTE_TYPE == $vote_type)
			{
				if ($this->has_voted($cookie_value, $item_id, self::UP_VOTE_TYPE))
				{
					$vote_value = self::SINGLE_ITEM_VOTE_TYPES[$vote_type] * -1;
				...
				{
					$vote_value = self::SINGLE_ITEM_VOTE_TYPES[$vote_type] +  (self::SINGLE_ITEM_VOTE_TYPES[self::DOWN_VOTE_TYPE] * -1); 
			...
			else
			{
				...
				{
					$vote_value = self::SINGLE_ITEM_VOTE_TYPES[$vote_type] +  (self::SINGLE_ITEM_VOTE_TYPES[self::UP_VOTE_TYPE] * -1); 
					$cookie_value = $this->remove_voted($cookie_value, $item_id, self::UP_VOTE_TYPE);
					$vote_type_action = self::REDO_VOTE_TYPE;
				}
			}
			if (isset($vote_value))
			{
				...
				$votes_count = $this->update_vote($item_id, $vote_value);
				setcookie( $cookie_name, json_encode($this->set_voted($cookie_value, $item_id, $vote_type)),time() + self::ONE_DAY_IN_SECS, '/');
				$voted = 1;
			}
			$cookie_value = $this->get_cookie_value($cookie_name);
