...
class Mv_List_Comparision
{
    ...
    public function setup_variables($compareId)
    {
        ...
        if(count($this->compareditems) > 1){
            ...
            $this->industry_averages = $this->calc_industry_averages($this->industry_items);
            echo "industry averages";
    ...
    public function get_column_betterthan($post_id, $item, $post_id2)
    {
        ...
        if ($reviews['list']['customersupport']['score'] > 4) {
            $r2b[] = "The quality of support has been rated to be better than ".get_the_title($post_id2);
        }
        ...
        // $price = (float) filter_var($price, FILTER_SANITIZE_NUMBER_FLOAT);//str_replace("$", "", $price);
        ...
        $price = get_field('price_starting_from', $post_id);
        $price = (float) filter_var($price, FILTER_SANITIZE_NUMBER_FLOAT);
        if ($price <= ($max / 2)) {
        ...
        $features_ratings2 = get_or_create_feature_ratings($post_id2);//get_field('features_list_ratings', $post_id);

		// print_r($total_score_feature_ind_avg);
		// print_r($features_ratings);
		$ffound=0;
		foreach($features_ratings as $f=>$r){
            if(isset($features_ratings2[$f])){
                if($r['average'] > $features_ratings2[$f]['average']){
                    $percent = round(($r['average']*100/$features_ratings2[$f]['average'])-100);
                    $r2b[]="When it comes to $f ".get_the_title($post_id)." has been rated $percent% above ".get_the_title($post_id2);// feature has been rated $percent% above industry average";
                    $ffound++;
                }
                if($ffound>2){
                    break;
        ...
        if (!empty($lists) && is_array($lists)) {
            ...
            foreach ($listrankord as $listid => $rank) {
                ...
                if ($rank < 4) {
                    ...
                    if($countLeaders>=2){
                        $seeMoreAppend = "<br><a href='#ranking'>See full list</a>";
                    }
                    $r2b[] = "Category leader in <a href='" . get_permalink($listid) . "'>" . $list_plural_title . "</a>".$seeMoreAppend;
                    $countLeaders++;
                } elseif ($rank < 11) {
                    $list_plural_title=get_list_title_acf($listid,'plural');
                    if($countLeaders>=2){
                    ...
                    $r2b[] = "A contender in <a href='" . get_permalink($listid) . "'>" . $list_plural_title . "</a>".$seeMoreAppend;
                    $countLeaders++;
                }
                if($countLeaders>2){
                    break;
        ...
        $sizef = 0;
        if(is_array($features)){
            $sorted_features_array= sort_features($features,$features_ids);
            $sizef = count($features);
		  }
		  else{
        ...
        $r2b[] = "Has " . $sizef . " highlight features" . $three;
        $htmlr2b = '<ul>';
		...
		<br>
        <?php
        echo has_feature_question($post_id,$this->industry_items);
		$con = ob_get_contents();
        ob_get_clean();
        return $con;
    }
    ...
    public function show_comparison_contnet()
    {
        ...
        <div class="comparison-box">
			...
			<div class="comparison-container content-have-<?php echo $compCount ?>">
                ...
                <div class="comparison_columns">
                    <div class="column-head">
						...
						//MAIN LOOP FOR SINGLE SECTIONS
						foreach ($basedthings as $it => $ittitle) 
						{
                            ob_end_flush();
                           /*  echo "inside $it memory used ";
                            echo memory_get_usage(); */
                            
							//PRICE VS PERFORMANCE
							if($it == "pricevsperformance")
							{
								...
								foreach ($comared_data as $post_idss) 
								{
									...
									$this_tooltip = '<div style=" margin: 10px;"><b>'.$this_title.'</b> <br> Performance : <b>'.$this_fs.'</b><br>Price : <b>$'.$this_price.$freqToPrint.'</b><br></div>';
									$arrToPassScatter[]=array($this_fs,$this_price,$this_title,$this_tooltip);
								} ?>
								...
								<script type="text/javascript">
									...
									var array2=<?php echo json_encode(array_values($arrToPassScatter)) ?>;
							...
							elseif ($it == "ratings") 
							{
								...
								foreach ($customArray as $upper_key => $single_customArray) 
								{
									foreach ($all_item_id as $all_items) {
										...
										/* echo "all support";
										print_r($all_support); */
										if(isset($all_support['list'][$upper_key]['score'])){
            ...
            elseif ($it == "features") {
                ...
                if(!is_array($features_ratings1)){
                    $features_ratings1= array();
                }
                if(!is_array($features_ratings2)){
                    $features_ratings2= array();
                }
                ...
                foreach ($all_item_id as $single_all_item_id) {
                    ...
                    foreach ($attributes_of_charts as $single_attributes_of_charts) {
                        ...
                        if (array_key_exists($single_attributes_of_charts, $item_features)) {
                            if(isset($item_features[$single_attributes_of_charts]['average'])){
                                if ($item_features[$single_attributes_of_charts]['average'] == 0) {
                                    continue;
                                }
                                if(!isset($features_ind_avg[$single_attributes_of_charts])){
                                    $features_ind_avg[$single_attributes_of_charts]= array('total_score'=>0,'count'=>0);
                                }
                              
                                $features_ind_avg[$single_attributes_of_charts]['total_score'] += 1;//$item_features[$single_attributes_of_charts]['average'];
                ...
                echo $item_two;
                print_r($get_scores2);
                echo "ind avg ratings";
                print_r($ind_avg_ratings);
                echo "artopas";
                print_r($arrToPass); */
                ?>
...
if (isset($_SESSION['act'])) {
                    ...
                    if(is_array($attributes_of_charts)){
...
foreach ($features_ratings1 as $f => $r) {
                    ...
                    foreach ($vAppArray as $feature_list_id) {
                        ...
                        foreach ($list as $key => $value) {
                            ...
                            if(isset($features_ratings[$fn_]['average'])){
...
$support = get_field('support', $post_id);
                ...
                if(isset($item_support_score_2['list']['customersupport']['score'])){
...
">Alernative</a>
                    ...
                    echo '<a class="mes-lc-li-down aff-link " href="' . $this->ctaArr[$this->winner]['afflink'] . '" rel="nofollow" target="_blank" style="
											font-size: 2em;
											margin: 0 auto;
											display: flex;
											width: fit-content;
...
$content = ob_get_contents();
        ...
        foreach ($compareditem as $ind => $cmp) {
            ...
            echo '<div style="display:table-cell; vertical-align:middle" ><span class="cp-item cp-item' . ($n + 1) . '" title="' . get_the_title($cmp) . '"> <img style="max-width:50px;" src="' . $image1 . '" class="img-responsive sss" alt="' . get_the_title($cmp) . '" ></span></div>';
...
$con = ob_get_contents();
    ...
    function calc_industry_averages($industry_items){
        $averages = array();
...
if (sizeof($this->ctaArr) < 2) {
    ...
    public function get_column_whofor($post_id, $item, $post_id2)
    {
        ob_start();
        $bf = get_field('best_for',$post_id);
        echo $bf;
...
/*                     echo "https://www.googleapis.com/youtube/v3/videos?key=$api&part=snippet&id=" . $videos_list[$i];
                    var_dump($http_response_header); */
                    $jsondec = json_decode($content);
                    // $jsondec = json_decode($ytarr['player_response'],true); ?>
                                            <!-- <div class="vid-item" data-videolink="http://youtube.com/embed/<?php echo $videos_list[$i]; 
                                            ?>?autoplay=1&rel=0&showinfo=0&autohide=1" onclick='reload()'> -->
                                            <?php if(isset($jsondec->items)){ ?>
    ...
    public function get_column_ranking($post_id, $item, $post_id2 = null)
    {
        ...
        if (!empty($lists) && is_array($lists)) {
            ...
            foreach ($listrankord as $id => $rank) {
				...
				$plural_title = get_field('list_content_title_plural', $id);
				if ($plural_title == '')
				{
					$plural_title = get_the_title($id);
				}
				else{
					$plural_title = 'Best ' . $plural_title;
				}
                echo '<li data-mh="equal-h-rank-' . $count . '" data-id="' . $id . '"><a class="compare_link ls_referal_link" data-parameter="comparison" data-id="' . $this->comparison_slug . '"  href="' . get_the_permalink($id) . '">' . $rankhtm . '<span class="rank-text">' . $plural_title . '</span></a></li>';
                $count++;
            }
            echo '</ul>';
        }
...
$heplerVar = 1;
    ...
    public function calculate_fs($post_id, $post_id2)
    {
      ...
      /*   echo "get overall combined rating";
        ...
        if(is_array($featurelist1)){
            $n1 = count($featurelist1);
        }
       
        if(is_array($featurelist2)){
            $n1 = count($featurelist2);
        }
