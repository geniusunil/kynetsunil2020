...
<?php
    ...
    $html_before_form .= '<div class="metabox-field">';
    $html_before_form .= '<label for="">Features</label><div class="meta-field-inp"><div id="list_items_features_meta">';
    ...
    ">+ Add another downloadable </span></div>
        ...
        <div class="yesnobtns" id="couponButtons"><button type="button" id="yesbtn" >yes</button>
        ...
        // <!-------------------------------------------qa start ------------------------------------------------->
        $qustion_list   = get_post_meta($post_id, 'qas_list', true);
        
        $html_before_form .= '<div ><h2>Add another FAQ</h2>
        <h5>Answer prospects most frequently asked questions.</h5>
        <div class="yesnobtns" id="qaButtons"><button type="button" id="yesqabtn" >yes</button>
        <button type="button" id="noqabtn" >no</button></div>';
        if(empty($qustion_list)){
            $class="hidden";
        }        
        else{
            $class="";
        }
            $html_before_form .= '<div id="qaForm" class="'.$class.' coupons_container_qa post_edit_page"><span class="add_new_qa">Add Another Qa</span> <ul class="list_all_qa qa_list">
 </ul> ';
        if(!empty($qustion_list)){    
            if(!empty($qustion_list) && is_array($qustion_list)){        
                $html_before_form .= '<div id="features">    
                     </div>';    
            
                $html_before_form .= "<ul class='post_coupons_list list1'>";
    
                // $couponlist = $couponlist[0];
                foreach ($qustion_list as $qustion_lists){                   
                    if(!empty($qustion_lists)){                        
                        $html_before_form .= '<li><span class="remove_qa">X Remove QA</span></li><li><p> Question:  <input type="text" name="old_question[]" value="'.$qustion_lists[question].'" required></p>
                        <p> Answer: <input type="text" name="old_answer[]" value="'.$qustion_lists[answer].'" required></p>
                        </li>'; 
                        }
                ...
                $html_before_form .= "<div class='no_features'>failed.</div>";
    
            }
        }else{        
                $html_before_form .= "<div class='no_features'>No FAQs entered!</div>";
        
            }
            $html_before_form .= '<div class="coupons_container_qa post_edit_page"><span class="add_new_qa">Add Another Qa</span> <ul class="list_another_cpn qa_list"></ul></div>';
...
autocomplete_software(document.getElementById("software_search1"), search_list);	 
    ...
    jQuery(document).ready(function () 
    {
        ...
        function unhideAndAddDownlod()
        {
            ...
            jQuery("#downloads").append('<div class="newfile"> <input type="file" name="kv_multiple_attachments[]"  required multiple="false"/> <input type="text" name="kv_multiple_attachment_titles[]"  >'+
            '<input type="file" name="kv_multiple_attachment_thumbs[]" accept="image/x-png,image/gif,image/jpeg" multiple="false"/>'+
        ...
        jQuery(document).on('click','#yesqabtn',function () 
        {
            ...
            jQuery(".list_all_qa").append('<li><span class="remove_qa">X Remove QA</span></li><li class="coup_code"><label> Question : </label><input type="text" name="new_question[]" value="" required> </li>'+
            '<li><label> Answer : </label><input type="text" name="new_answer[]" value="" required> </li>');
                    
        });
        jQuery(document).on('click','#noqabtn',function () 
        {
            jQuery("#qaForm").addClass("hidden");
            console.log("hide");
        });
...
jQuery(document).on('click','.remove_coupon',function () {
    ...
    jQuery(document).on('click','.remove_qa',function () {
        // jQuery(this).closest('li').remove();
        while (jQuery(this).closest('li').next().length){
            jQuery(this).closest('li').next().remove();
...
jQuery(document).on('click','.add_new_qa',function () {
    console.log(jQuery(this).closest('.coupons_container_qa').find('ul'));
    jQuery(this).closest('.coupons_container_qa').find('ul').append('<li><span class="remove_qa">X Remove QA</span></li> <li><label> Question : </label><input type="text" name="old_question[]"  value="" required> </li>'+
    ' <li><label> Answer : </label><input type="text" name="old_answer[]"  value="" required> </li>'
   );
    console.log("show");
