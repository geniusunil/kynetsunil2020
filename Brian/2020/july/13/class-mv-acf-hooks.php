...
class Mv_Acf_Hooks 
{
	...
	function change_dir_thumbs( $dir ) 
	{
		$update_post_id = $_SESSION['update_post_id'];
		return array(
			'path'   => $dir['basedir'] . '/listcreator/attachments//'.$update_post_id.'/thumbs',
			'url'    => $dir['baseurl'] . '/listcreator/attachments//'.$update_post_id.'/thumbs',
			'subdir' => '/listcreator/attachments//'.$update_post_id.'/thumbs',
		) + $dir;
	}
	...
	function my_save_post( $post_id ) 
	{
		...
		{ 
			...
			$filesThumbs = $_FILES["kv_multiple_attachment_thumbs"]; 
			$filesTitles = $_POST['kv_multiple_attachment_titles'];
			...
			{ 			
				...
				{ 
					...
					{				
						$newupload = $this->kv_handle_attachment($file,$post_id); 
					}
					
					
					remove_filter( 'upload_dir', array( $this, 'change_dir' ) );
					file_put_contents("pluginhooks.txt","check 1 ",FILE_APPEND);
					$origFileName = basename ( get_attached_file( $newupload ) );
					$lastIndexofDot = strrpos($origFileName,'.');
					$thumbName = substr($origFileName,0,$lastIndexofDot);
					if(isset($filesThumbs['name'][$key])){
						file_put_contents("pluginhooks.txt","check 2 ",FILE_APPEND);
						
						$lastIndexofDot2 =  strrpos($filesThumbs['name'][$key],'.');
						$extension = substr($filesThumbs['name'][$key],$lastIndexofDot2);
						$finalThumbName = $thumbName.$extension;
						$filethumb = array( 
							'name' => $finalThumbName,
							'type' => $filesThumbs['type'][$key], 
							'tmp_name' => $filesThumbs['tmp_name'][$key], 
							'error' => $filesThumbs['error'][$key],
							'size' => $filesThumbs['size'][$key]
						); 
						file_put_contents("pluginhooks.txt","check 3 ".print_r($filethumb,true),FILE_APPEND);

						$_FILES = array ("kv_multiple_attachment_thumbs" => $filethumb); 
						add_filter( 'upload_dir', array( $this, 'change_dir_thumbs' ) );
						file_put_contents("pluginhooks.txt","check 4 ".print_r($_FILES,true),FILE_APPEND);

						foreach ($_FILES as $filethumb => $array) 
						{				
							$newupload = $this->kv_handle_attachment($filethumb,$post_id); 
							file_put_contents("pluginhooks.txt","new upload second time \n".print_r($newupload,true),FILE_APPEND);

						}
						remove_filter( 'upload_dir', array( $this, 'change_dir_thumbs' ) );
					}
					file_put_contents("pluginhooks.txt",content_url('uploads/listcreator/attachments/'.$post_id.'/thumbs'),FILE_APPEND);

					if(isset($filesTitles[$key])){
						file_put_contents("pluginhooks.txt","check text ",FILE_APPEND);
						$finaltxtName = $thumbName.'.txt';
						file_put_contents(content_url('uploads/listcreator/attachments/'.$post_id.'/thumbs')."/$finaltxtName",$filesTitles[$key],FILE_APPEND);
