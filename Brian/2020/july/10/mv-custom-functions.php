<?php
	...
	*/
	...
	function generate_list_html($this_list,$ajax=true,$sort='',$current_page=1){
		echo "hellooooo";
		// var_dump($ajax);
		ob_start();
...
<?php }?>
								...
								<div class="background_softwareFinderRadar" style="width: 70%;height: 500px;margin: 0 auto;">
										...
										<script type="text/javascript">
											...
											function addBG<?php echo $key; ?>()
											{
												...
												// var svg = document.querySelector(".radarChart svg");
												// console.log({svg});
												var attsRect = document.querySelector("#radarChart<?php echo $key; ?> svg").children[1].children[0];
												console.log({attsRect});
												var parentElement = document.querySelector("#radarChart<?php echo $key; ?> svg").children[1];
												console.log("here");
	...
	/* 	echo "avgnof ".$avgnof." ";
		...
		$votes = get_post_meta($post_id,'votes_given_singleitem',true);//get_field('votes_given_singleitem',$post_id);
	/* 	echo "votes";
		print_r($votes); */
		if(is_array($votes)){
			if(!isset($votes['up'])){
			...
			$upcent = round($votes['up']*100/($votes['up']+$votes['down']));
			$downcent=100-$upcent;
			if($upcent > 80){
				$r2b[]="$upcent% of users say they would recommend ".get_the_title($post_id);
			}
			else{
				$rn2b[]="$downcent% of users say they wouldn't recommend ".get_the_title($post_id);
			}
		}
