...
class Mv_List_Single_View_New
{
    ...
    private $overlist;
    private $bestoverallfree;
    private $bestoverall;
    private $generated_list;
    private $finalVotes_jbs;
    private $industry_averages;
    ...
    public function set_list_vars($list_id)
    {
        $this->reviewClass = new RWP_Rating_Stars_Shortcode();
        $this->finalVotes_jbs = do_shortcode("[total_votes id=$list_id]");
        $this->generated_list = generate_list($list_id);
        $generated_list = $this->generated_list;
        foreach($generated_list[''] as $key=>$value)
        {
            $this->bestoverall = $key;
            break;
        }
        
        foreach($generated_list['user_friendly'] as $key=>$value)
        {
            $this->bestcustomerservice = $key;
            break;
        }
        foreach($generated_list['affordable_price'] as $key=>$value)
        {
            $this->bestbudgetoption = $key;
            break;
        }
        foreach($generated_list['free'] as $key=>$value)
        {
            $this->bestoverallfree = $key;
            break;
        }
	...
	public function get_list_faq()
	{
        ...
        $free_software_id =  $this->bestoverallfree;
		$free_software_title = get_the_title($free_software_id );
		$faq1_ans = $free_software_id . "Our users have voted that $free_software_title is the preferred free $faq1.";
		// faq-1 END
		...
		{
            $customListArray = $this->generated_list[''];
            /* echo "gen list";
            print_r($this->generated_list); */
			foreach($customListArray as $key=>$value)
			{
			...
			$faq.= " <div itemscope itemprop='mainEntity' itemtype='https://schema.org/Question'>
				...
				<div itemscope itemprop='acceptedAnswer' itemtype='https://schema.org/Answer'>
				<div itemprop='text'>
					Ans. The results are in and over a ".$this->finalVotes_jbs." as cast their votes and ".get_the_title($topProductWithFeature)." is the recommended choice.
				 </div>
				</div>
			</div>"; 
		}
		...
		$compareitems= array();
		foreach($this->generated_list[$sorts] as $key=>$value)
		{
			$compareitems[]=$key;
	...
	public function featured_section_block($list_id)
	{
		ob_start();
		?>
		<div class="container-fluid featured">
			<div class="container2">
				...
				$all5features = array($this->bestPromoted, $this->bestoverall, $this->bestcustomerservice, $this->bestbudgetoption, $this->bestoverallfree);
				?>
				<div class="row">
					<?php
					$widthClass = "col-3";
        ...
        if (is_singular('lists') && !has_shortcode($content, 'show_list')) 
		{
            ...
            if (!has_shortcode($post->post_content, 'show_list')) 
			{
				...
				foreach($all_sections as $single_all_section)
				{
					...
					{
						$content .=$this->featured_section_block(get_the_ID());
					}
		...
		elseif (is_singular('list_items')) 
		{
            ...
            foreach($all_sections as $single_section)
			{
				...
				elseif($single_section == 'prosAndCons') //done
				{
					...
					$rnr2b      = get_pros_cons($post_id,$this->total_score_feature_ind_avg,$this->best_in_industry);
				    $r2b = $rnr2b['r2b'];
					$rn2b = $rnr2b['rn2b'];
					$htmlr2b = '<ul>';
					foreach ($r2b as $r) 
				...
				elseif($single_section == 'marketOverview')
				{
                    $content .= $this->market_overview($post_id);
				}
    ...
    public function market_overview($post_id)
    {
        ob_start();
        ?>
        <div class ="item-overview-content" id="marketradar">
            <h3>SoftwareFindr Market Radar</h3>
            <?php $filters = decide_filters($post_id); ?>
            <select class='dropdown-one' id="select_radar" style="border:1px solid black;float: right;position: relative;z-index: 99;">
                <?php 
            ...
            <script>
                var hide_show_radar = function()
                {
                    ...
                    jQuery('#marketradar > div').each(function()
                    {
            ...
            {
                generate_radar_chart($post_id,$key,$arr);
            }
            ?>
        </div>
        <?php 
    ...
    public function set_single_list_item_vars($post_id)
    {
        ...
        //GET INDUSTRY AVG
        $this->industry_averages = calc_industry_averages($post_id, $this->industry_items);
        //GET BEST FOR ITEM
        $this->best_in_industry = get_or_refresh_best_for($post_id,$this->industry_averages);
    }
							...
							<table class="table table-striped" style="box-shadow: 0 0 22px rgba(0,0,0,0.20);margin-top: 1rem;">
								<thead>
									<tr>
                                        ...
                                        foreach ($postarr as $item_id=>$score) 
                                        {
                                            ...
                                            $item_link_title = "<a class='text-dark' href='".get_the_permalink($item_id)."'>$item_title</a>";
                                            if (has_post_thumbnail( $item_id ) )
                                            {
                                                $image = wp_get_attachment_image_src( get_post_thumbnail_id( $item_id ), 'single-post-thumbnail' );
                                            }
                                            else 
                                            {
                                                $image = '';
                                            ...
                                            //FINDSCORE
                                            $item_findscore = get_or_calc_fs_individual($item_id);
                                            $item_findscore_html = '<p><b> '. $item_findscore .' % </b></p>';

                                            //table button for item 
                                            $onlyAffiliate_url = get_field('affiliate_url', $item_id);
                                            if ( ! empty( $onlyAffiliate_url ) ) 
                                            { 
                                                if(substr_count($onlyAffiliate_url, "?")>=1){
                                                    $onlyAffiliate_url.="&utm_source=softwarefindr.com&utm_medium=free%20traffic&utm_campaign=".$item_id;
                                                }
                                                else
                                                {
                                                    $onlyAffiliate_url.="?utm_source=softwarefindr.com&utm_medium=free%20traffic&utm_campaign=".$item_id;
                                            ...
                                            $onlySource_url = get_field('source_url', $item_id);
                                            $onlyAfflink = empty( $onlyAffiliate_url )? $onlySource_url:$onlyAffiliate_url;

                                            $onlyAffiliate_button_text = get_field('affiliate_button_text', $item_id) == '' ? 'Download/Demo' : get_field('affiliate_button_text', $item_id);
                                            $onlyCredit_text = get_field('credit', $item_id);
                                            $onlyBtntext = empty($onlyAffiliate_button_text) ? $onlyCredit_text : $onlyAffiliate_button_text;


                                            $availability = get_post_meta($item_id, '_item_availbility', true);
                                            if($availability == 'no') 
                                            { 
                                                $button = '<a href="'. get_the_permalink( $item_id ).'alternative/" data-id="'. $item_id .'" data-parameter="itemid"><button type="button" class="btn btn-success btn-sm">INFO</button></a>';
                                            }
                                            else 
                                            { 
                                                $button = '<a href="'. $onlyAfflink.'"><button type="button" class="btn btn-success btn-sm">INFO</button></a>';
                                            } 
                                            ...
                                            $rating_item = $this->reviewClass->get_stars( $overall, 20, 5 );
                                            $itemReview[$item_id] = '<div class="title-review" style="width: 100%;margin: 0 auto;"> ' . $rating_item . ' </div>';


                                            $firstRow = $table_css . $item_image_html . $item_title_html . $item_findscore_html . $button . $item_compare;
                                            ?>
                                            <th scope="col"><?php echo $firstRow;?></th>
                                            <?php
                                            $price_data = get_field('price_starting_from', $item_id, false);
                                            ...
                                            //GET OVERALL COMBINE RATINGS BY MULTIPLE PRODUCTS
                                            $combineRatings[$item_id] = get_overall_combined_rating($item_id);

                                            //GET VOTES 
                                            $productVotes[$item_id] = get_post_meta($item_id,'votes_given_singleitem',true);
                                            
                                            //REASON FOR QUIT
                                            $quitVotes[$item_id] = get_post_meta($item_id, 'quit_votes', true);
								...
								<tbody>
                                    <tr>
										...
										<?php 
										foreach ($itemReview as $key => $single_itemReview) 
										{
											echo '<td> ' . $single_itemReview . '</td> ';
										} ?>
									</tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                        foreach ($combineRatings as $key => $vlm) 
										{
											echo ' <td>' . $vlm['list']['valueformoney']['score']*2 . '/10</td>';
										} ?>
                                    ...
                                    <tr>
                                        ...
                                        foreach ($combineRatings as $key => $vlm) 
										{
											echo ' <td>' . $vlm['list']['featuresfunctionality']['score']*2 . '/10</td>';
										} ?>
                                    ...
                                    <tr>
                                        ...
                                        foreach ($combineRatings as $key => $vlm) 
										{
											echo ' <td>' . $vlm['list']['easeofuse']['score']*2 . '/10</td>';
										} ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($combineRatings as $key => $vlm) 
										{
											echo ' <td>' . $vlm['list']['customersupport']['score']*2 . '/10</td>';
										} ?>
                                    </tr>
                                    <tr>
                                        <th scope="row" class="text-left">Likelihood of recommending</th>
                                        <?php
                                        foreach ($productVotes as $key => $vlm) 
										{
                                            if(!empty($vlm))
                                            {
                                                $upcent = round($vlm['up']*100/($vlm['up']+$vlm['down']));
                                                echo '<td>' .  $upcent  . ' % </td>';
                                            }
                                            else
                                            {
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                        foreach ($quitVotes as $key => $whyleave) 
										{
                                            if(!empty($whyleave))
                                            {
                                                $maxs = array_keys($whyleave, max($whyleave));
                                                echo ' <td>' . $maxs[0] . '</td>';
                                            }
                                            else
                                            {
									...
									<tr>
										...
										{
                                            $all_feature_list_score[$key] = get_or_create_feature_ratings($key);
										...
										{
											echo '<tr><th class="text-left">' . $k . '</th>';
											foreach ($feature_list as $j => $items) 
											{
												if(is_array($items))
												{
													if (in_array($k, $items)) 
													{
                                                        $score = $all_feature_list_score[$j][$k]['total_score'];
                                                        if($score > 7)
                                                        {
                                                            $cellcolor = 'bg-success';
                                                        }
                                                        elseif($score <= 3)
                                                        {
                                                            $cellcolor = 'bg-danger';
                                                            ...
                                                            $cellcolor = 'bg-warning';
                                                        }
                                                        
                                                        echo '<td class="'.$cellcolor.'"><i class="fa fa-check" aria-hidden="true" style="color: green;" ></i> <p>'.$score.'</p></td>';
													}
													else 
													{
...
">Promoted</span>
    ...
    public function item_content_value($post_id)
    {
        ...
        $item_result .= '<div id="overview"><div class="overview-content item">' . $contentP . '</div></div><a href="javascript:;" class="readbutton">Read More >>></a>';
        $item_result .= '</div></div>';
        return $item_result;
    }
...
$compObj = new Mv_List_Comparision(null,true);
    ...
    public function get_single_list_item_content($post_id, $content)
    {
        ...
        foreach ($all_item_id as $all_items) 
		{
            $price_trim[] = (float) filter_var($price, FILTER_SANITIZE_NUMBER_FLOAT);
        ...
        $title = get_the_title($post_id) . " Reviews: Pricing and Features";  $rat = get_overall_combined_rating($post_id);
        $nor = $rat['count']; 
        ...
        $item_feature_rating = get_or_create_feature_ratings($item_id); 
        ...
        $ease_average                       = $this->industry_averages['easeofuse'] * 2;
        $functionality_feature_average      = $this->industry_averages['functionality'] * 2;
        $valueformoney_feature_average      = $this->industry_averages['valueformoney'] * 2;
        $customersupport_feature_average    = $this->industry_averages['customersupport'] * 2;
        $item_rat = 0;
        ...
        $details .= '<div class="row">
                        <div class="col-sm-12">
                            ...
                            $details .= '<div class="col-sm-12"> <div class="rating-count">' . $rating_item . '</div>
                                        ...
                                        if($this->availability == 'no') 
                                        { 
                                            $details .='<a style="color: white;" href="'. get_the_permalink( $post_id ).'alternative/" class="alter-btn head-links" data-parameter="itemid" data-id="'. $post_id .'" >Alernative</a>';
                                        }
                                        else 
                                        { 
                                            $details.='<a style="color: white;" class="mes-lc-li-down head-links" href="'. $this->afflink.'" rel="nofollow" target="_blank">'.$this->btntext.'</a>'; 
                                        } 
                                    $details.='</p>';
                                    ...
                                    $details .= '<div class="item-description price_feature ">
                                                    ...
                                                    $details .= '<div class=" review-sections">Value for money<br>
                                                                ...
                                                                // echo'<pre>';
                                                                // print_r($this->industry_averages);
                                                                // print_r($this->best_in_industry);
                                                                // echo'</pre>';
        $details .= '<strong>Best For: </strong> <ul class="feature_list">';
        foreach($this->best_in_industry as $single_best_ind)
        {
            $single_best_ind = trim($single_best_ind);
            $details .= '<li><i class="fa fa-square"></i>'. $single_best_ind .'</li>';
        }
        ...
        /*------------------------------------------------------section1 complte-----------------------------------------------------------*/
        $details .= '<div class="row" style="margin-bottom: 5%;"><hr class="hr-line" style="border-top: 7px solid #e1e3e3;width: 100%;">
                        <div style="display: flex; position: absolute; margin-left: 28%;">';

        if ($nor > 10) 
        {
        $details .= '<img class="leaf1" src="' . esc_url(plugins_url('image/leaf1.png', dirname(__FILE__))) . '"><div class="row11" style="height: fit-content;">
