...
class Mv_List_Single_View_New
{
	...
	public function list_page_faqs()
	{
		// FAQ Start 
    ...
    public function show_list($content)
    {
        ...
        if (is_singular('lists') && !has_shortcode($content, 'show_list')) 
		{
            ...
            if (!has_shortcode($post->post_content, 'show_list')) 
			{
				...
				foreach($all_sections as $single_all_section)
				{
					...
					if($single_all_section == '')
					{
						
					}
					
					elseif($single_all_section == '')
					{
                ...
                $content .= $this->list_contnet($post_id);
				//$content .=$this->featured_section_block();
				//$content .= $this->another();
				//$content .= $this->list_page_faqs();
		...
		elseif (is_singular('list_items')) 
		{
			...
			//PAGE STARTS HERE
			$content  = $this->get_single_list_item_content(get_the_ID(), ''); //done
            ...
            foreach($all_sections as $single_section)
			{
				...
				{
					$rnr2b=get_pros_cons($post_id,$this->total_score_feature_ind_avg);
					$r2b = $rnr2b['r2b'];
					$rn2b = $rnr2b['rn2b'];
					$htmlr2b = '<ul>';
					foreach ($r2b as $r) 
    ...
    public function list_compare_table($postarr)
    {
        ...
        $i = 0;
        $list_comparion_data = '<p style=" font-size: 1.75rem; font-family: ProximaNova !important;">The ' . get_the_title($post_id) . ' Summed Up</p>';
        ?>
