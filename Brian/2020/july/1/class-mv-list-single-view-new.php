...
class Mv_List_Single_View_New
{
    ...
    public function show_list($content)
    {
		...
		elseif (is_singular('list_items')) 
		{
			...
			$content = $this->get_single_list_item_content(get_the_ID(), '');
            ...
            foreach($all_sections as $single_section)
			{
				...
				{
					$content .=  $this->adaptation_of_geo_map($post_id) ; //used $this->adaptation_of_geo_map($post_id); instead of list_geo_map()
				}
				...
				{
					$content .= $awardSection;
				}
				
				elseif($single_section == 'keyFeatures')
				{
					$content .= '<div class="item_list_page_container">' . $this->keyfeature_pricing($post_id) . '</div>';
				...
				elseif($single_section == 'pricing')
				{
					...
					//pricing result
					$content .= '<div class="pricing-item-new left-content-box style="
				...
				{
					$content .= '<div class = "item-softwarefindr-radar item-sec-div">';
		...
		/*	$content .= '</div>'; //full-widthdiv
			
			
			$content .= '<div class="item_list_page_container">' . $this->data_item_page($post_id) . '</div>';
			
			
			$content .= '' . $this->videos_sec($post_id) . '';
			
			
			$content .= '<div class="item_list_page_container">' . $this->support_sec($post_id) . '</div><div id="reviews"></div>';
			 */
...
$html = ob_get_contents();
    ...
    public function get_award_list($post_id)
    {
        ...
        $award_list=array();
        if (!empty($listrankord)) {
            $ranked_result = '<div class = "item-overview-content">';
            $ranked_result .= '<div class = "item-sec-div" >';
        $ranked_result .= '<h3>Ranking & Positions </h3>';
        $ranked_result .= '</div>'; // item-sec-div
        $ranked_result = '</div>'; // item-overview-content
        $ranked_result .= '</div>'; //item-content-sec1 div
        $ranked_result .= '</div>'; //item-overview-content div
        $ranked_result .= '</div>'; //item-body-content
        $ranked_result .= '</div>'; //full-width full item container
            foreach ($listrankord as $id => $rank) {
