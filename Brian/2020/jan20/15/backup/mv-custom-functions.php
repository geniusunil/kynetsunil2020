...
function itempros_func( $atts ) {
    ...
    // file_put_contents("debug.txt",print_r($a,true));
...
function get_item_ranks($pId){
    ...
    //file_put_contents("debug.txt","I am inside get_item_ranks\n",FILE_APPEND);
...
function populate_both_r2b_arrays($post_id){
    ...
    $max = 0;
    // echo "priceses are : ";
    //file_put_contents("debug.txt","I am inside get_single_list_item_content3\n",FILE_APPEND);
    ...
    foreach($lists as $alternate){
        ...
        //file_put_contents("debug.txt","I am inside get_single_list_item_content8\n",FILE_APPEND);
    ...
    //file_put_contents("debug.txt","I am inside get_single_list_item_content8 ltrim value is :".$trimmedprice." maxhalf value is $maxhalf if value is :".$ifvalue."\n",FILE_APPEND);

    //file_put_contents("debug.txt","I am inside get_single_list_item_content8 if value is :".($trimmedprice <= $maxhalf)."\n",FILE_APPEND);

    if($trimmedprice <= $maxhalf){
        //file_put_contents("debug.txt","I am inside get_single_list_item_content9\n",FILE_APPEND);

        $r2b []= "Compared to others the price is reasonable"; 
        //file_put_contents("debug.txt","I am inside get_single_list_item_content10\n",FILE_APPEND);

    }
    else{
        //file_put_contents("debug.txt","I am inside get_single_list_item_content11\n",FILE_APPEND);
        
        $rn2b []= "A bit on the Expensive side compared to other solution in this category.";
        //file_put_contents("debug.txt","I am inside get_single_list_item_content12\n",FILE_APPEND);

    }
        //file_put_contents("debug.txt","I am inside get_single_list_item_content7\n",FILE_APPEND);
    ...
    //file_put_contents("debug.txt","I am inside get_single_list_item_content6\n",FILE_APPEND);
    ...
    //file_put_contents("debug.txt","I am inside get_single_list_item_content5\n",FILE_APPEND);
            ...
            foreach ($qalist as $qa){
                ...
                if(!empty($qa)){
                    // for($j=0;$j<count($qa);$j++){
                            // print_r($qa);
                            $qatypeqa = ($qa[type]=="qa")? "selected":"";
                            $qatypedeal = ($qa[type]=="deal")? "selected":"";
                            $ccclass = ($qatypedeal=="selected")?"hidden":"";
                            $ccrequired = ($qatypedeal=="selected")?"":"required";
                             $qadateshow = ($qa[showDate]=="show")? "selected":"";
                            $qadatehide = ($qa[showDate]=="hide")? "selected":"";
                           /* $cdclass = ($qadatehide=="selected")?"hidden":""; */
                            echo '<li><label class="main_label"> qa : </label><span class="remove_qa">X Remove qa</span></li>';
                            echo ' <li><label> qa Type : </label></label><select class="coupdeal" name="old_qa_type[]" ><option value="qa" '.$qatypeqa.'>qa</option><option value="deal"  '.$qatypedeal.'>Deal</option></select></li>';
                            
                            echo '<li class="'.$ccclass.'" data-mh="feat_li"><label> qa code : </label> <input type="text" name="old_qa_code[]"  value="' . $qa[code]. '" id="feat_'.$key.'" '.$checked.' '.$ccrequired.'></li>';
                            echo '<li><label> Link : </label><input type="text" name="old_qa_link[]" value="'.$qa[link].'" required> </li>';
                            echo '<li data-mh="feat_li"><label>Discount Amount/Text :</label> <input type="text" name="old_qa_title[]"  value="' . $qa[title]. '" id="feat_'.$key.'" '.$checked.' required></li>';
                            echo '<li data-mh="feat_li"><label> Description : </label> <input type="text" name="old_qa_description[]"  value="' . $qa[description]. '" id="feat_'.$key.'" '.$checked.' required></li>';
                            echo '<li><label> qa/Deal Expiration : </label></label><select class="exptoggle" name="old_qa_expdate_show[]"><option value="show" '.$qadateshow.'>show</option><option '.$qadatehide.' value="hide">Hide</option>
                            
                          </select></li>';
                            echo '<li data-mh="feat_li " class = "exp_date"> <label> Expiry date : </label><input type="text" name="old_qa_expiry_date[]"  value="' . $qa[expdate]. '" id="feat_'.$key.'" '.$checked.' required></li>';
					
                            echo '<li data-mh="feat_li"> <input type="hidden" class="immutable" name="old_qa_id[]"  value="' . $qa[id]. '" id="feat_'.$key.'" '.$checked.'></li>';
					
                            echo '<li data-mh="feat_li"> <input type="hidden" class="immutable" name="old_qa_votes[]"  value="' . $qa[votes]. '" id="feat_'.$key.'" '.$checked.'></li>';
 ...
 */
 ...
 function re_save_qa_meta_boxes($post_id, $post, $update){
    ...
    $titles = $_POST['old_qa_title'];
    $descriptions = $_POST['old_qa_description'];
    $codes = $_POST['old_qa_code'];
    $dates = $_POST['old_qa_expiry_date'];
    $ids=  $_POST['old_qa_id'];
    $votes = $_POST['old_qa_votes'];
    $qa_types =  $_POST['old_qa_type'];
    $qa_links = $_POST['old_qa_link'];
    $dates_show = $_POST['old_qa_expdate_show'];
    $oldArr = array();

    for($i=0;$i<count($titles);$i++){
        if($titles[$i] != '' ){
            $oldArr []= array('type'=>$qa_types[$i],
            'link'=>$qa_links[$i],
            'title'=>$titles[$i],
            'description'=>$descriptions[$i],
        'code'=>$codes[$i],
    ...
    if(isset($_POST['new_qa_title']) && is_array($_POST['new_qa_title']) && !empty($_POST['new_qa_title'])){
        $qa_types =  $_POST['new_qa_type'];
        $qa_links = $_POST['new_qa_link'];
        $titles = $_POST['new_qa_title'];
        $descriptions = $_POST['new_qa_description'];
        $codes = $_POST['new_qa_code'];
        $dates_show = $_POST['new_qa_expdate_show'];
        $dates = $_POST['new_qa_expiry_date'];
        // $newArr = array_filter($newArr);
        $nextid=0;
        foreach($meta_box_value as $key=>$mbv){
            $toExp=$mbv[id];
...
$id = $my_posts[0]->ID;
    $allDealPosts = get_post_meta($id,'all_posts_with_qa');
...
//    echo "all deal posts are ".print_r($allDealPosts);
    ...
    unset($allDealPosts[$key]);
	         ...
	         update_post_meta($id, 'all_posts_with_qa',$allDealPosts);
