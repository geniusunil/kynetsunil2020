    ...
    public function has_feature_question($post_id, $item, $post_id2){
        ...
        foreach($sorted_feature_list as $sfl){
            if(!in_array($sfl['feature'],$alreadyChecked) && !in_array($sfl['feature'],$currentFeatures)){
                $cf = $sfl['feature'];
                echo "does this item has ".$sfl['feature']." feature? <button onclick='send_feature_vote(\"yes\",\"$cf\",\"$post_id\")'>yes</button><button onclick='send_feature_vote(\"no\",\"$cf\",\"$post_id\")'>no</button>";
                ?>
...
$compareditemnew = array();
        ...
        <script>
                    function send_feature_vote(vote,feature,post_id){
                        // zf_post_id=123;
                        // feature_name='test';
