...
<?php }?>
	...
	foreach($sorted_feature_list as $sfl){
		if(!in_array($sfl['feature'],$alreadyChecked) && !in_array($sfl['feature'],$currentFeatures)){
			$cf = $sfl['feature'];
			$html.= "<span style='margin:auto 0;'>Does this item has ".$sfl['feature']." feature?</span> <button class='feature_ask' onclick='send_feature_vote(\"yes\",\"$cf\",\"$post_id\",this)'>yes</button><button class='feature_ask' onclick='send_feature_vote(\"no\",\"$cf\",\"$post_id\",this)'>no</button>";
			?>
