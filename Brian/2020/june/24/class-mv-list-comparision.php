...
class Mv_List_Comparision
{
    ...
    public function has_feature_question($post_id, $item, $post_id2){
            $i=0;
            ...
            $alreadyChecked = get_field('already_checked_no_feature',$post_id);
        ...
        foreach($sorted_feature_list as $sfl){
            if(!in_array($sfl['feature'],$alreadyChecked) && !in_array($sfl['feature'],$currentFeatures)){
                echo "does this item has ".$sfl['feature']." feature? <button onclick='send_feature_vote(\"yes\")'>yes</button><button onclick='send_feature_vote(\"no\")'>no</button>";
                ?>
    ...
    public function get_column_betterthan($post_id, $item, $post_id2)
    {
		...
		<br>
        <?php
        $this->has_feature_question($post_id, $item, $post_id2);
		$con = ob_get_contents();
        ob_get_clean();
        return $con;
    }
...
$compareditemnew = array();
        ...
        <script>
                    function send_feature_vote(vote){
                        zf_post_id=123;
                        feature_name='test';
                        jQuery.ajax(
                            {
                                url: '<?php echo admin_url('admin-ajax.php') ?>',
                                type: 'POST',
                                data: {post_id: zf_post_id, feature_name : feature_name, vote : vote, action: 'send_feature_vote'},
                                dataType: 'json',
