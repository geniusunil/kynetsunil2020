       ...
       The '.get_the_title($post_id).' Summed Up
        ...
        <?php
                ...
                foreach($lists as $post_idss)
                    {             
                        $item_name = $post_idss;
                        $features_ratings1[$post_idss] = get_field( 'features_list', $post_idss );
                        // echo $key;
                       
                        foreach($features_ratings1 as $key =>$features){  
                            ...
                            // $topfeature_avg = $features;
                            // echo  $topfeature_avg;
                            // print_r($features);
                            $title = '  <tr><th>'.$features[0].'</th>' ;
                                echo $title;
                                echo '<td> '.get_the_title($key).'</td> </tr>';
                            $j++;
                            if($j == 20)
                            break;
                            }
  ...
  <tbody>
    ...
    foreach($feature_list as $key =>$features_list){
        ...
        foreach($features_list as $list){
            ...
            $frequent_used = array_count_values($list);
            echo "list";
                        print_r($frequent_used);


            echo '<th scope="row">'.$list.'</th>';
            // echo "<td>yes</td>";

            // if(in_array($list, array_values($feature_list))  ){
            // echo '<td>'.$key.'</td>';
...
// if( $key )
        ...
        if (in_array($list, $features_list))
            {
                //prints the inner array if the inner array contains your search 
               echo "<td>yes</td>";
            } 
            else{
...
/******************************************** faq-2 start ***********************************************/
    ...
    function list_decision_sec($post_id){
        ...
        ?>
            ...
            <span> Pay cost attention to the FindrScore given as it incorporates over [number of data points for this list] data points powered by <?php  echo $finalVotes; ?> user voices. Narrow down and use the comparison feature to figure out which is right for you.</span>
            </div>
            </div>
            </div>
            </div>
...
// echo "awaard list";
                        ...
                        foreach($all_award_list as $key => $item_ind_list){                           
                            $award_items[] = "<h4 style='color: #fff;'>".get_the_title($key)."</h4>";
        ...
        $key_feature_result .= '<script>
                 ...
                 data: {';              
                   foreach($item_rating_feature as $f => $r){ 
                       ...
                       $f_ = $this->validate_var_name($f); //str_replace(" ","_",$f);                         
                       $score = $r[average];
...
/* -----------------------------------------------get_single_sidebar fxn close--------------------------------------------------------------*/
    ...
    function get_single_list_item_content( $post_id, $content ) {
            ...
            $rating_item = do_shortcode( '[rwp_users_rating_stars id="-1"  template="rwp_template_590f86153bc54" size=20 post='.$post_id.']' );            
            $nor = $this->get_number_of_reviews($rating_item);
            // echo "nor is ";
        // echo $nor;
        /* if($nor > 1){
