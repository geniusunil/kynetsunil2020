...
if (!class_exists('Mes_Lc_Ext')){
	class Mes_Lc_Ext{
		...
		const ITEM_FEATURE_RATING_COOKIE_NAME = 'mes-lc-feature-rate_';
		...
		function get_cookie_value_feature($cookie_name){
			$cookie_value = isset($_COOKIE[$cookie_name])? $_COOKIE[$cookie_name]: false;
			file_put_contents("get_feature.txt","cookie name : ".print_r($_COOKIE[$cookie_name],true)." \n ",FILE_APPEND);
		...
		function on_feature_rate(){
			...
			// if (array_key_exists($vote_type,self::VOTE_TYPES)){
				$cookie_name = self::ITEM_FEATURE_RATING_COOKIE_NAME . $item_id.'_'.$feature_name;
				...
				file_put_contents("on_feature.txt"," cookie_value : ".print_r($cookie_value,true)." \n ",FILE_APPEND);
				file_put_contents("on_feature.txt","cookie name : ".print_r($cookie_name,true)." \n ",FILE_APPEND);
				...
				$cur_feature_score = $this->update_feature_rating($item_id, $feature_name, $vote_size);
					...
					setcookie($cookie_name, 
		    			...
		    			time() + self::ONE_DAY_IN_SECS, '/');
				     	$voted = 1;
				// }
				file_put_contents("meslcext.txt","cur_feature_score : ".print_r($cur_feature_score,true),FILE_APPEND);
		...
		function set_voted($data, $item_id, $vote_type){
			...
			if ($vote_type && array_key_exists($vote_type,self::VOTE_TYPES))
				...
				file_put_contents("set_voteitem.txt","array".print_r($data,true),FILE_APPEND);
			return $data;
		}
		function set_voted_feature($data, $item_id, $vote_size,$feature_name){
			...
			// if (!isset($data[$vote_size])) $data[$vote_size] = array();
			// if (!is_array($data[$vote_size])) $data[$vote_size] = array();
			// if ($vote_size && array_key_exists($vote_size,self::VOTE_TYPES))
				$item_value = array($item_id,$feature_name);
				file_put_contents("set_vote.txt","array".print_r($item_value,true),FILE_APPEND);
			return $data;
		}
		...
		/* 	if (!is_array($featureListratings)){
				$featureListratings = array();
			} */
			$featureListratings[$feature_name]['votes']+=1;
