		...
		{         
            ...
            <script>
				var hide_show = function()
				{
					...
					//for paragraph
					var check_1 = $('#'+first_option+'_1').val();
					var check_2 = $('#'+first_option+'_2').val();
					if(check_1 > check_2)
					{
						var attr_win = <?php echo json_encode($item_one); ?>;
					}
					else
					{
						var attr_win = <?php echo json_encode($item_two); ?>;
					}
					var conclusionPara = '<p>'+attr_win+' is Easy to Use </p>';
			...
			<div id="showhide001">
				<div id="series_chart_div_easeofuse_valueformoney" style="width: 900px; height: 500px;" class="chartShow"></div>
				<div id="series_chart_div_valueformoney_customersupport" style="width: 900px; height: 500px;" class="hide_chart "></div>
				<div id="series_chart_div_valueformoney_featuresfunctionality" style="width: 900px; height: 500px;" class="hide_chart "></div>
				<div id="series_chart_div_featuresfunctionality_easeofuse" style="width: 900px; height: 500px;" class="hide_chart "></div>
				<div id="series_chart_div_featuresfunctionality_customersupport" style="width: 900px; height: 500px;" class="hide_chart "></div>
				<div id="series_chart_div_customersupport_easeofuse" style="width: 900px; height: 500px;" class="hide_chart "></div>
			</div>
		   ...
		   <script>
			...
			<div id="conclusionPara">
				<?php
					$first_criteria = json_encode($item_one); 
					if(json_encode($valueformoney_2) > json_encode($valueformoney_1))
					{
						$first_criteria = json_encode($item_two); 
					}
					if($first_criteria == json_encode($item_one))
					{
						if(json_encode($easeofuse_1) > json_encode($easeofuse_2))
						{
							$second_criteria = 'Set';
						}
					}
					...
					{
						if(json_encode($easeofuse_2) > json_encode($easeofuse_1))
						{
							$d_selected_item = json_encode($item_one);
						}
					}
				?>
				<p><?php echo $first_criteria; ?> offers the best value for money <?php if(isset($second_criteria)) { echo 'and is also the is easier to use'; } else { echo 'but is more expensive than '.$d_selected_item .' '; } ?> — deciding between the two is a question of tradeoffs.</p>
			</div>
...
foreach($feature_all_ratings as $key => $fv)
{
	...
	echo 'Array To Pass';
	print_r($arrToPass);
	echo 'final_attributes_of_chart';
	print_r($final_attributes_of_chart);
...
///////////////////////////////////////////// --end-- ///////////////////////////////////////////////////////                     
                ...
                $passToMaps = array();
                // $passToMaps[]= array('Country', 'Value', json_encode($object));
