...
/******************************************** faq-2 start ***********************************************/
    ...
    // function integrate_with($post_id){
    //     $lists =  get_field( 'integrate_with_item', $post_id, false );
    //     $itemiid = $post_id;
    //     $integrate_item = array();
    //     if ( !empty ( $lists ) && is_array( $lists ) ) {
    //         foreach ( $lists as $id ) {
    //             if($this->acme_post_exists($id)){
    //                 $rank = get_item_rank($id,$itemiid);
    //                 $integrate_item[$id] = $rank;
    //             }
    //         }
    //         asort($integrate_item);
    //         print_r($integrate_item);
    //     }   
...
<div>
                   ...
                   <canvas id="marksChart" width="600" height="400"></canvas>
                    <script>
                        ...
                        var marksData = {
                            labels: <?php echo json_encode($all_feature_name); ?>,
         ...
         <div id="app">
                <v-app>
           <?php foreach($item_rating_feature as $f => $r){
               ...
               ?>
                ...
                ></v-slider>
                    <span class="status">{{<?php echo $f_ ?>.status}}</span>
               ...
               var vm = new Vue({
                  ...
                  data: {
                    ...
                    foreach($features_ratings as $f => $r){ 
                        ...
                        $score = $r[average];
                        echo "$f_ : {label : '$label', val : $score, color: 'green' ,status:''}" ?>,
                  <?php } ?>
                      
                    }, watch : {
                        Site_Customization:function(val) {
                  console.log("watch");
               }},
                    methods: {
    ...
    //   alert('Hello ' + this.name + '!')
      // `event` is the native DOM event
        ...
        var zf_post_id = document.querySelectorAll('.zf-item-vote')[0].getAttribute("data-zf-post-id");
        ...
        $item_result .='<div class="row"><div class="col-sm-12">';
        ...
        $item_result .='<div class="col-sm-6" > <div class="col-sm-5 integrate_img">';
        $item_result .=  $integarte_item_img;
        $item_result .=' </div >';
        $item_result .=' <div class="col-sm-6 integrate_name">';
        $item_result .=  "<strong><span>".$integarte_item_name."</strong></span>";
        $item_result .=' </div></div>';  
    ...
    foreach($all_item_id as $all_items){       
     ...
     $all_support_score_avg = array_sum($all_support_score)/count($all_item_id);
     
     $all_support_score_per = array_sum($all_support_score)*100/ count($all_item_id);

//    echo  count($all_item_id);
//    print_r($all_support_score_avg);
    //  $coupn_count = count($all_list);
    //  $coupon_all_list = 
    //  foreach($all_support as $key =>$support_items){
   ...
   foreach($comared_data as $post_idss)
    ...
    // print_r($result);
   // <!-- /************************************************************chart******************************************************************* */ -->
        $item_result .='<div class="pricing-item-new left-content-box style="
...
// $p2show_open_source = $freemium_percentage;
// $open_source_percentage = $subscription_percentage+$p2show_open_source;
// $colorOfOpenSourcePercentage = "grey";
// $open_source_percentage_title = "Other";
...
<div class="viewport inview">
  <svg width="100%" height="666px" viewBox="217 116 554 666" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">';
  $item_result .= '<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" transform="translate(218.000000, 116.000000)"><g><g transform="translate(336.000000, 98.000000)"></g></g><g transform="translate(250.000000, 169.000000)" stroke="#FFFFFF" stroke-width="2"><path d="M0,0 L101,67"></path><path d="M6,224 L107,291" transform="translate(56.500000, 257.500000) scale(-1, 1) translate(-56.500000, -257.500000) "></path>';
  $item_result .= '</g><text font-size="20" line-spacing="25" fill="#FFFFFF"><tspan x="100" y="19">Pricing Model</tspan></text><g transform="translate(89.000000, 32.000000)"><g><circle r="88" cx="88" cy="88" fill="#e8d743" />';
//   $item_result .= '<circle r="44" cx="88" cy="88" fill="transparent" stroke="'.$colorOfOpenSourcePercentage.'" stroke-width="88" transform="rotate(-90) translate(-176)"stroke-dasharray="calc('.$open_source_percentage.' * 276.32 / 100) 276.32"/>';
//   $item_result .= '<rect x="200" y="20" width="20" height="20" rx="5" fill="'.$colorOfOpenSourcePercentage.'" /><text class="detail-text" font-size="20" line-spacing="20" fill="#5BD49C"><tspan x="230" y="40">'.$open_source_percentage_title.' '.$p2show_open_source.'%</tspan></text>';
 
  $item_result .= '<circle r="44" cx="88" cy="88" fill="transparent" stroke="'.$colorOfSubscriptionPercentage.'" stroke-width="88" transform="rotate(-90) translate(-176)"stroke-dasharray="calc('.$subscription_percentage.' * 276.32 / 100) 276.32"/>';
  $item_result .= '<rect x="200" y="45" width="20" height="20" rx="5" fill="'.$colorOfSubscriptionPercentage.'" /><text class="detail-text" font-size="20" line-spacing="20" fill="#5BD49C"><tspan x="230" y="65">'.$subscription_percentage_title.' '.$p2show_subscription.'%</tspan></text>';
  $item_result .= '<circle r="44" cx="88" cy="88" fill="transparent"stroke="'.$colorOfFreemiumPercentage.'"stroke-width="88"stroke-dasharray="calc('.$freemium_percentage.' * 276.32 / 100) 276.32"transform="rotate(-90) translate(-176)" />';
  $item_result .= '<rect x="200" y="70" width="20" height="20" rx="5" fill="'.$colorOfFreemiumPercentage.'" /><text class="detail-text" font-size="20" line-spacing="20" fill="#5BD49C"><tspan x="230" y="90">'.$freemium_title.' '.$p2show_freemium.'%</tspan></text>';
  $item_result .= '<circle r="44" cx="88" cy="88" fill="transparent"stroke="'.$colorOfOneTimePercentage.'" stroke-width="88" stroke-dasharray="calc('.$one_time_license_percentage.'  * 276.32 / 100) 276.32" transform="rotate(-90) translate(-176)" />';
  $item_result .= '<rect x="200" y="95" width="20" height="20" rx="5" fill="'.$colorOfOneTimePercentage.'" /><text class="detail-text" font-size="20" line-spacing="20" fill="#5BD49C"><tspan x="230" y="115">'.$one_time_license_percentage_title.' '.$one_time_license_percentage.'%</tspan></text></g></g>';
  $item_result .= '<text font-size="20" line-spacing="25" fill="#FFFFFF"><tspan x="366" y="185">Promotional Offers</tspan></text><g transform="translate(324.000000, 197.000000)"><circle fill="#b2e8d6" stroke="#fff" stroke-width="2" cx="114" cy="114" r="114" class="circ-fill" id="r15"></circle>';
  $item_result .= '<circle r="57" cx="62" cy="114" fill="transparent" stroke="#bbb0b0" stroke-width="114" stroke-dasharray="calc('.round($coupon_all_list).' * 357.9 / 100) 357.9" transform="rotate(-90) translate(-176)" />';
  $item_result .= '<text class="detail-text" font-size="20" line-spacing="25" fill="forestgreen"><tspan x="135" y="86">'.round($coupon_all_list).'%</tspan></text> </g><text font-size="20" line-spacing="25" fill="#FFFFFF"><tspan x="80" y="364">Free Trials</tspan></text>';
  $item_result .= '<g transform="translate(0.000000, 377.000000)"><circle fill="#c2d2e0" stroke="#fff" stroke-width="2" cx="132" cy="132" r="132" class="circ-fill-circle" id="r16"></circle><circle r="66" cx="44" cy="132" fill="transparent" stroke="#bbb0b0" stroke-width="131" stroke-dasharray="calc('.round($free_trail_percentage).' * 414.48 / 100) 414.48" transform="rotate(-90) translate(-176)" /> ';
  $item_result .= '<text class="detail-text" font-size="20" line-spacing="25" fill="forestgreen"><tspan x="141" y="76">'.round($free_trail_percentage).'%</tspan></text>';
  $item_result .= '</g></svg></div></div>';
...
?>
    ...
    //     data.addRows(
    //         <?php //echo json_encode(array_values($result)) ?>
    ...
    //    print_R($item_support_score);
        if($all_support_score_avg > $item_score){           
            ...
            $item_result .='<img src="https://area52.softwarefindr.com/wp-content/uploads/2020/02/bad-exprience.png">';
        }
        $item_result .= '</div>';
        $item_result .= '<div class = "col-sm-4">';
        
        if($all_support_score_avg > $item_support_score){   
            $item_result .='satisfaction rating which indicates that you will be in good company with $item_title_name';
        ...
        $item_result .= '<p>Looking at data gathered on our platform the industry average for this category is
...
//    $mutual_feature_name = $key;
//    $mutual_feature_score = $feature_one;
    break;
    }
// echo "mutual feature";
// print_r( $mutual_feature_name);
// // print_r( $mutual_feature_score);


//  echo"</pre>";


// if($item_feature_individual_avg[$key] ===  $total_score_feature[$key] ){

//     echo "true";
//     print_r($item_feature_individual_avg[$key]);

// }else{
//     echo "features not match";
// }

    // 
    // echo "Industry feature <pre>";
    // print_r($total_score_feature);
    // echo "</pre>";

    // echo "total score";
    // print_r($total_score);
    // $total_score_sum = array_sum($total_score_feature);
    // echo "total score sum".$total_score_sum;

    // $total_score_avg =  $total_score_sum/count($all_item_id)*2; // all features score arry
