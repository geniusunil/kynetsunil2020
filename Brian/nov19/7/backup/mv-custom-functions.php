...
function alter_coupon_alternative_page_description( $description ) {
    ...
    if ( !empty( $item_name ) ) {
        ...
        if ( is_page( 'coupon' )){
            ...
            $description = str_replace( '[Item name]', $post->post_title, $page_title );
...
function alter_comparision_page_title( $title ) {
        ...
        $title = str_replace( '[Item name]', get_the_title($post->ID), $page_title );
        $title = str_replace( '[Year]', date('Y'), $title );
...
function title_shortcode_filter( $title ){
	global $post;
 		...
 		$title = str_replace( '[Item name]', get_the_title($post->ID), $title );
        $title = str_replace( '[Year]', date('Y'), $title );
...
function description_shortcode_filter( $description ){
		global $post;
 		...
 		$description = str_replace( '[Item name]', get_the_title($post->ID), $description );
        $description = str_replace( '[Year]', date('Y'), $description );
		...
		if(!empty($couponlist) && is_array($couponlist)){
            ...
            foreach ($couponlist as $coupon){
                ...
                if(!empty($coupon)){
					...
					$coupondatedisplay = ($coupon[coupon_used]=="selected")? "selected":"";
                            $cdclass = ($coupondatehide=="selected")?"hidden":"";
                            echo '<li><label> Coupon : </label></li>';
                            echo ' <li><label> Coupon Type : </label></label><select class="coupdeal" name="old_coupon_type[]" ><option value="coupon" '.$coupontypecoupon.'>Coupon</option><option value="deal"  '.$coupontypedeal.'>Deal</option></select><span class="remove_coupon">X</span></li>';
                            
                            echo '<li class="'.$ccclass.'" data-mh="feat_li"><label> Coupon code : </label> <input type="text" name="old_coupon_code[]"  value="' . $coupon[code]. '" id="feat_'.$key.'" '.$checked.'></li>';
                            ...
                            echo '<li class="'.$cdclass.'"data-mh="feat_li"> <label> Expiry date : </label><input type="text" name="old_coupon_expiry_date[]"  value="' . $coupon[expdate]. '" id="feat_'.$key.'" '.$checked.' required></li>';
					
					
					
                            echo '<li data-mh="feat_li"> <input type="hidden" name="old_coupon_id[]"  value="' . $coupon[id]. '" id="feat_'.$key.'" '.$checked.'></li>';
                            echo '<li data-mh="feat_li"> <input type="hidden" name="old_coupon_votes[]"  value="' . $coupon[votes]. '" id="feat_'.$key.'" '.$checked.'></li>';
...
add_action( 'wp_ajax_nopriv_my_action', 'coupon_date' );
add_action( 'wp_ajax_my_action', 'coupon_date' );
...
$postiddata = $_POST['postiddata'];
	...
	$resp = array("postiddata" => $postiddata);
