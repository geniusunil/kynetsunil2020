            ...
            foreach ($couponlist as $coupon){
                ...
                if(!empty($coupon)){
                    // for($j=0;$j<count($coupon);$j++){
                            ...
                            echo '<li data-mh="feat_li"><label> Title : </label> <input type="text" name="old_coupon_title[]"  value="' . $coupon[title]. '" id="feat_'.$key.'" '.$checked.'></li>';
                            echo '<li data-mh="feat_li"><label> Description : </label> <input type="text" name="old_coupon_description[]"  value="' . $coupon[description]. '" id="feat_'.$key.'" '.$checked.'></li>';
                            echo '<li data-mh="feat_li"><label> Coupon code (if any) : </label> <input type="text" name="old_coupon_code[]"  value="' . $coupon[code]. '" id="feat_'.$key.'" '.$checked.'></li>';
                            echo '<li data-mh="feat_li"> <label> Expiry date : </label><input type="text" name="old_coupon_expiry_date[]"  value="' . $coupon[expdate]. '" id="feat_'.$key.'" '.$checked.'></li>';
 ...
 */
 ...
 function re_save_coupon_meta_boxes($post_id, $post, $update){
    ...
    for($i=0;$i<count($titles);$i++){
        if($titles[$i] != '' ){
            $oldArr []= array('title'=>$titles[$i],'description'=>$descriptions[$i],
        'code'=>$codes[$i],
        'expdate'=>$dates[$i],
        'id'=>$ids[$i],
        'votes'=>$votes[$i]);
    ...
    if(isset($_POST['new_coupon_title']) && is_array($_POST['new_coupon_title']) && !empty($_POST['new_coupon_title'])){
        ...
        for($i=0;$i<count($titles);$i++){
            if($titles[$i] != '' ){
                $newArr []= array('title'=>$titles[$i],'description'=>$descriptions[$i],
            'code'=>$codes[$i],
            'expdate'=>$dates[$i],
            'id'=>$nextid,
            'votes'=>0);
