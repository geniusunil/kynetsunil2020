...
<!--			<input type ="text" value="search" >-->
	...
	<?php //	var_dump($allDealPosts);
		...
		<div class="cat_deal"><select >		
		<?php	foreach($allDealPost as $alllist){
...
if($i<4){
		...
		echo "<div class='cpn_list'><a data-id='$cat_id' data-url= '$cat_link' data-name= '$term->name' class='all_cat_name' data-page='$current_page'  data-search = '$deal_search' data-post ='$var'>".$term->name."</a></div>";
...
//		print_r($couponlist);
	...
	<div class="view_coupon"><div class="all_coupn_btn" id ="view_coupn_btn" data-startpage="<?php echo $current_page+1 ?>"  data-uri="<?php echo $_SERVER['REQUEST_URI']; ?>"> <?php echo $current_page+1; ?> View More Coupon</div> </div>
...
//	var cat_name = jQuery(this).data("name");	
		...
		var search_term = jQuery(this).data("search");
		var termid = jQuery(this).data("id");
		
	ajax_call_coupons(current_page,search_term,termid);
	jQuery("#deal_search_submit").data("id",termid);
	jQuery("#view_coupn_btn").data("id",termid);
...
//	var cat_name = jQuery(this).data("name");	
	...
	ajax_call_coupons(current_page,search_term,termid);
	jQuery(".all_cat_name").data("search",search_term);
	jQuery("#view_coupn_btn").data("search",search_term);
	...
	jQuery("#deal_search").keyup(function() {
		...
		jQuery("#deal_search_submit").data("search",jQuery(this).val());
	});
});
	...
	//------------------------------------View More------------------------------------------------
	jQuery(".all_coupn_btn").click(function(){
		
	var current_page = jQuery(this).data("startpage");
