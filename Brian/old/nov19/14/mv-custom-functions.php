...
//    echo "all deal posts are ".print_r($allDealPosts);
    ...
    unset($allDealPosts[$key]);
	         ...
	         update_post_meta(148162, 'all_posts_with_coupon',$allDealPosts);
...
$post_per_page = 10;
	...
	$couponlist  = get_post_meta($post_id, 'coupons_list', true);
		file_put_contents("mvvv.txt",print_r($couponlist ,true),FILE_APPEND);
			...
			if($search_term == ''  || strpos(strtolower(get_the_title($post_id)),strtolower($search_term))!== false )
				...
				foreach($category_list as $category){
					...
					if($category->term_id == $cat_id){
						...
						file_put_contents("mvvv.txt","category".print_r($category_list ,true),FILE_APPEND);
...
//			file_put_contents("mvvv.txt","filtered_coupons ".print_r($filtered_coupons ,true),FILE_APPEND);
			...
			$filtered_coupons= array_slice($filtered_coupons,$start_index ,$post_per_page);
//			file_put_contents("mvvv.txt","filtered_coupons ".print_r($filtered_coupons ,true),FILE_APPEND);
	
		 $resp = array("filtered_coupon"=> $filtered_coupons, "current_page_no" => $current_page_no, "category_name" =>$category,"search_item" => $search_term,"term_id" => $cat_id, "link_url" => $url);
