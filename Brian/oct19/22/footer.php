...
<script>
	...
	<?php } ?>
var infinite_scroll = function(list_id, reuri, count, location = '') {
    			jQuery.ajax({
							url: "<?php bloginfo('wpurl') ?>/wp-admin/admin-ajax.php",
							type:'POST',
							data: {"action": "infinite_scroll", "page_no": count, "list_id": list_id, "location": location},
							beforeSend: function() {
	...
	console.log("location : "+location);
		if(countl > total_pages) {
			 ...
			 return false;
						   ...
						   infinite_scroll(list_id, reuri, countl,location);				 
						   countl++;
		  }
...
jQuery('#infinite-loader').on('inview', function(event, isInView) {
	...
	if (isInView) {
		if(count <= total_pages){
			 infinite_scroll(list_id, reuri, count,location);			 
             count++;
		} else {
			jQuery("#nopostmessage").html("<p style='text-align:center'>No more posts!</p>");
			jQuery(this).hide();
