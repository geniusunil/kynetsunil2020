...
get_header();
 $targetCountries=array(
    'IN' => 'India',
    'US' => 'United States',
    'GB' => 'United Kingdom'); // array of countries for which we want to show modified lists and change url attribute ?lang=IN
    ...
    if($langPos == false)
        ...
        if(array_key_exists($isoCode,$targetCountries)){
            ...
            <script>
                ...
                history.replaceState({name: 'Location'}, "pushState example", "?lang=<?php echo $isoCode ?>");
...
">
    ...
    $bestoverall = $bestcustomerservice = $bestbudgetoption = $att_items_id_only[0];
    $bestoverallscore = $bestcustomerservicescore = $bestbudgetoptionscore = $bestoverallfreescore = 0;
    $bestoverallfree = "no free option.";
...
////
    ...
    if ($bestoverallfree == null) {
        $all4features = [$bestoverall, $bestcustomerservice, $bestbudgetoption];
    } else {
...
<script>
    ...
    jQuery('#options a').on('click', function() {
        ...
        var newC=jQuery(this).data("val");
        var newCname=jQuery(this).text();
        if(newC != ''){
            history.replaceState({name: 'Location'}, "pushState example", "?lang="+newC);
                 // if (document.title != newTitle) {
            document.title = "<?php echo $origTitle ?> in "+newCname+" | SoftwareFindr";
            jQuery('meta[property="og:title"]').attr("content","<?php echo $origTitle ?> in "+newCname+" | SoftwareFindr");
            // }
            jQuery('.sliderheader').text("<?php echo $origTitle ?> in "+newCname);
        }
