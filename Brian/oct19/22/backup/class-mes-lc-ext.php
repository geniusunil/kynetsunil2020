...
if (!class_exists('Mes_Lc_Ext')){
	class Mes_Lc_Ext{
		...
		function on_list_item_vote(){
			...
			// file_put_contents("meslcext.txt","LOCATION FROM post".print_r($location,true));
			...
			if (array_key_exists($vote_type,self::VOTE_TYPES)){
				...
				if (self::DOWNLOAD_VOTE_TYPE == $vote_type){
					if(!$this->has_voted($cookie_value, $item_id, $vote_type)){
						...
						$this->register_download_with_location($item_id,$location);
					}
				}	
		...
		function register_download_with_location($item_id,$location){
