...
//								  $html = str_replace('rel="stylesheet"','rel="stylesheet" media="none" onload="if(media!=\'all\')media=\'all\'"',$html);
                    ...
                    //  echo "<pre>$message</pre>";
                    if(trim($datee) == '1571290973'){
                      file_put_contents("evmails_view.txt","non trash /n".$message);
                    }
   ...
   <?php $datee = get_Contents($sample, '<Date>', '<endDate>');
      ...
      <div class="main_msg <?php echo "mail".$i; ?>">
               ...
               <?php $eid = get_Contents($sample, '<eid>', '<endeid>');
                   ...
                   $message  = get_Contents($sample, '<message>', '<endmessage>');
                   if(trim($datee) == '1571290973'){
                    file_put_contents("evmails_view_t.txt","trash 2/n".$message,FILE_APPEND);
                  }
                   $file_ids = get_Contents($sample, '<id>', '<endid>');
                   $file_id  = $file_ids;
                   $a        = $file_id;
                   ...
                   $html        = $message;
                    ...
                    while (($lastPos  = strpos($html, $needle, $lastPos))!== false) {
                     ...
                     if(trim($datee) == '1571290973'){
                      file_put_contents("evmails_view_t.txt","trash 1/n".$message,FILE_APPEND);
                    }
...
jQuery(".email_view").click(function () {
     ...
     data:{'useremail': emails, 'senderemail': senderemails, 'message': 'no need', 'subject': scontent, 'file_name': file_name, 'e_date': edate, 'f_name': fname, 'attachments': attachment, 'attachment_pdf': attach_pdf, 'attachment_doc': attach_doc, 'cust_ids': false, 'pageno': '<?php echo $this->input->get('page'); ?>' },
