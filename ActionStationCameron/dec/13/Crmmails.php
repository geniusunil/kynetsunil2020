...
public function loadViewmails()
	...
	public function viewmails()
    {
        ...
        $this->session->set_userdata('page', $this->input->get('page'));
        // file_put_contents("/debug2.txt", "running viemails" . PHP_EOL, FILE_APPEND);
        // file_put_contents("crmmails.txt", "page get is : " . $this->input->get('page'). PHP_EOL, FILE_APPEND);
  ...
  $incoming_email = $this->accounts_model->systems_datas($id);
        ...
        if($ustype == "businessaccountmanager"){
          ...
          elseif($ustype == "businessaccount"){
           ...
           echo  $file                 = $folder.$data['filename'];
            $file1                = basename($file);
            $countprocess = substr_count($file1, "process_");
            if($countprocess < 1){
                $date = "process_";
                 if (file_exists($file)) {
                    $file_name = $folder.$date.trim($file1);
		...
		<table>
			...
			{ 
				...
				if( $userInfo != Null)
				{
					...
					<tr> 
						<td> 
							<form action="<?php echo base_url('/crmmails/viewentry_1') ?>" method="post" class="select_users">
	...
	public function do_not_traks()
    {
		...
		{  	
			...
			$file1        = basename($file);
			$date         = "trashed_";
			$counttrash = substr_count($file, "trashed_");
			file_put_contents("crmmails.txt","counttrash is $counttrash\n",FILE_APPEND);
			if($counttrash < 1)
			{
	...
	public function restore_trash_email()
	{
		...
		$file_name = $this->input->post('id');
		file_put_contents("crmmails.txt","restore deleted text of $file_name\n",FILE_APPEND);
		...
		foreach($file_name as $filename1)
		{
			...
			if (file_exists($file)) 
			{
				$file_name1 = $folder.trim($filename);
