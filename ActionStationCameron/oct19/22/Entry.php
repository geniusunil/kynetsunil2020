	...
	$pathss = "email_files/".$db."/".$username;
	 ...
	 $pathss = "email_files/".$db."/".$username;
			...
			if($action !== false){
				...
				$saved_mail =  $mailto."ExclusiveLineBreak".$date."ExclusiveLineBreakSubject: ".$subject."ExclusiveLineBreak".$body.$attachments.$notes;
...
////////////////////////////////////////////
	...
	public function update_entry_notes()
	{
		...
		if($dbname !="")
		 ...
		 $entry                   = $this->input->post('entryId');
		 $notes                   = $this->input->post('entry_notes');
		 $old_notes               = $this->input->post('old_note');
		 $data['action_notes']    =  $notes;
         $updated                 = $this->entry_model->update_entry_notes($entry, $data);
		 
		if($updated !== false)
		{
							$action 		                =   array();
							$action['entry_id'] 		    = 	$entry;
							$action['action_type']			=   24;
							$action['action_source'] 		= 	base_url('dashboard');
							$action['action_direction'] 	= 	"Internal";
							$action['action_status'] 		= 	"Completed";
							$action['action_notes'] 	    = 	$old_notes;
