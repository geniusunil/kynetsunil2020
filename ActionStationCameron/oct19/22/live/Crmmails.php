 ...
 public function do_not_traks()
    {
        ...
        file_put_contents("crmmails.txt","inside do+not traks\n",FILE_APPEND);
	...
	$pathss = "email_files/".$db."/".$username;
     ...
     $folder     =  $pathss.'/';
            ...
            foreach($file_name1 as $filename ){  	
			    ...
			    $file1        = basename($file);
                $date         = "trashed_";
                file_put_contents("crmmails.txt","DO NOT TRAKS : folder, date and filename : $folder, $date and $filename\n",FILE_APPEND);

                $file_name12  = $folder.$date.trim($filename);
                $oldDir       = $file;
                $newDir       = $file_name12 ;  
                file_put_contents("crmmails.txt","DO NOT TRAKS : olddir and newdir : $oldDir and $newDir\n",FILE_APPEND);
