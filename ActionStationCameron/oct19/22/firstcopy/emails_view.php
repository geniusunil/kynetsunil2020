...
.Trashed {  font-size: 14px !important; }
    ...
    <input type="checkbox" class="chk_boxes" label="check all"  />  <b>Select all on page</b>&nbsp;
...
<?PHP if ($ustype == 'businessaccountmanager') {
    ...
    file_put_contents($_SERVER['DOCUMENT_ROOT'] . "/action_station/debug.txt", "looking for email files at " . $dir . PHP_EOL, FILE_APPEND);
    $textwithprocessed = glob("{$dir}*.{txt}", GLOB_BRACE);

    $textonlyprocessed = glob("{$dir}trashed_*.{txt}", GLOB_BRACE);
    $text = array_diff($textwithprocessed, $textonlyprocessed);
    ...
    function sortByReceiveDate($text){
      ...
      // print_r($keyvaluetext);
      arsort($keyvaluetext);
      // print_r($keyvaluetext);
      return array_keys($keyvaluetext);
    }
    $text = sortByReceiveDate($text);
    $textonlyprocessed = sortByReceiveDate($textonlyprocessed);
    ...
    for ($i = $start; $i <= $end; $i++) { //loop for showing the emails one by one
       if ($text != "") {
        ...
        if ($arr[0] !== 'process') {
              // echo '<br>';	   
         if (!empty($file_name)) {?>
<div class="col-md-12 email_view"  id="abd" data-id="<?php echo $file_name ?>">
<style>
...
echo $dt->format('D, j F, Y, g:i a');  ?>
   ...
   <div class="main_msg <?php echo "mail".$i; ?>">
              ...
              <?php  $eid = get_Contents($sample, '<eid>', '<endeid>');
                     ...
                     file_put_contents($_SERVER['DOCUMENT_ROOT'] . "/action_station/debug.txt", "input.messages contains :" . htmlentities($message) ."input.messages ends"  . PHP_EOL, FILE_APPEND);
...
Leaking CSS on create user dialog
                    ...
                    $html = $message;
                     ...
                     echo $message; //for mails which are not trashed
                   ...
                   if ($ustypees == "businessaccountmanager") {
                     ...
                     echo '<input type="hidden" class="hide" id="img_id_path" value="' . $filees . '">';
                        ...
                        if ($filees) {
                            $directory  = $_SERVER['DOCUMENT_ROOT'] . '/action_station/' . $filees . '/';
                     file_put_contents($_SERVER['DOCUMENT_ROOT'] . "/action_station/debug.txt", "looking for attachments files at " . $directory . PHP_EOL, FILE_APPEND);
                     ...
                     file_put_contents($_SERVER['DOCUMENT_ROOT'] . "/action_station/debug.txt", "content of pdf variable is : " . print_r($pdf, true) . PHP_EOL, FILE_APPEND);
...
<?php file_put_contents($_SERVER['DOCUMENT_ROOT'] . "email_files/" . $dbname . "/" . $username . "/attachments/" . $a . "/emailsViewDebug.txt", "textfileID imagefileID equalOrNot : " . $text_file_id . " " . $image_file_id . " " . ($text_file_id == $image_file_id) . PHP_EOL, FILE_APPEND);
      file_put_contents($_SERVER['DOCUMENT_ROOT'] . "/action_station/debug.txt", "textfileID imagefileID equalOrNot : " . $text_file_id . " " . $image_file_id . " " . ($text_file_id == $image_file_id) . PHP_EOL, FILE_APPEND);
...
<?php
				...
				<?php   $count++;   }
                         ...
                         ?>
                           <div class="email_view1 view11">
                         <?php
                         foreach($textonlyprocessed as $top){ ?>
                          ...
                          <?php 
                              ...
                              // echo '<br>';	   
                               ...
                               $file_name;
                               if (!empty($file_name)) {?>
       <div class="col-md-12 email_view1 xxx"  id="abd" data-id="<?php echo $file_name ?>">
   <style>
   .chk_boxes {  margin-left: 3% !important; }
   </style>
        ...
        <div class="em_head">
        <div class='col-md-1 bbb'>
        <input name="checkboxes" type="checkbox" id="delcheck" class="delcheck" value="<?php echo $file_name ?>">
        </div>
   ...
   <?php $datee = get_Contents($sample, '<Date>', '<endDate>');
      ...
      <div class="main_msg <?php echo "mail".$i; ?>">
                   ...
                   <?php $eid = get_Contents($sample, '<eid>', '<endeid>');
                     ...
                     // end of body tag refining
                        $message = $html;
                         echo $message;  //for hidden trashed mails
...
<?php 
       ...
       if ($ind > 1) {
           echo "<a target='_self' class='btn btn-default create' href='viewmails?page=" . ($ind - 1) . "&no=" . ($ind1 - 10) . "' class='button'>Previous</a>";
           echo " ";
          }
       for ($i = 1; $i <= ceil($filecounts/10); $i++) {
		...
		if($i == $ind || $i == ($ind-1) ){	
            echo "<a target='_self' class='btn btn-default create " . $class . "' href='".base_url('crmmails/viewmails')."?page=" . $i . "&no=" . ($i * 10 - 10) . "' >" . $i . "</a>";
            echo " ";
			}
			elseif($i > ($ind+5) && $i <= $ind+10){
            echo ".";
			}
            elseif($i > (ceil($filecounts/10)- 2)){
            echo "<a target='_self' class='btn btn-default create " . $class . "' href='".base_url('crmmails/viewmails')."?page=" . $i . "&no=" . ($i * 10 - 10) . "' >" . $i . "</a>";
            echo " ";
			}elseif($i == (ceil($filecounts/10)-2)){
			 echo "...";		
        ...
        if ($ind < ceil($filecounts/10)) {
            echo "<a target='_self' class='btn btn-default create' href='".base_url('crmmails/viewmails')."?page=" . ($ind + 1) . "&no=" . ($ind * 10) . "' class='button'>Next</a>";
            echo " ";
        }
		    echo "<a target='_self' class='btn btn-default createt old' href='".base_url('crmmails/viewmails')."?page=" . ceil($filecounts/10) . "&no=" . (ceil($filecounts/10) * 10 - 10) . "' class='button'>Oldest</a>";
            echo " ";
    } ?>
<?php } ?>
<!-- end of pagination fxn -->
<!-- call to pagination fxn -->
<div class="col-md-12" style="margin-top:6%; text-align:left">          
<span class="page1">
<?php
