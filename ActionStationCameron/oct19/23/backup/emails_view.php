...
jQuery(function() {
    jQuery('.chk_boxes').click(function() {
        jQuery('.delcheck, .delcheck1').prop('checked', this.checked);
    });
});
</script>
...
<?PHP if ($ustype == 'businessaccountmanager') {
  ...
  // echo "dir2 is $dir";
  // echo "textwithprocessed before sanitization";
  // print_r($textwithprocessed);
    // sanitizeNames($textwithprocessed,$dir);
    // echo "textwithprocessed after sanitization";
    // print_r($textwithprocessed);
    ...
    function sanitizeNames(&$text,$dir){
      ...
      foreach($text as $t){
        ...
        $counttrash = substr_count($t, "trashed_");
        echo "counttrash is $counttrash";
       if($counttrash > 1){
          // $text[$i]=str_replace("trashed_","",$text[$i],$counttrash-1 );
        }
         /*if (file_exists($dir.$t)) {
          rename($dir.$t, $dir.$text[$i]);
          }
        $i++;*/
      } 
  } 
...
<?php }else{
        ...
        // echo  "cureent page no.:".$_SESSION['last_page_no'];
      ...
      if ($ind > 1) {
        ...
        // echo  "start and end: $start and $end";
