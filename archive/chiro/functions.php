...
function premium_listing_filter( &$posts, &$postvisible, $return = false ) {
  ...
  // print_r($posts);
	...
	if(count($premium_basic_items)) {
		...
		foreach( $premium_basic_items as $post ) {
...
function rate_premium_listing_filter( &$posts, &$postvisible, $return = false ) {
	file_put_contents(ABSPATH."functions.txt","posts".print_r($posts,true));
	file_put_contents(ABSPATH."functions.txt","postvisible".print_r($postvisible,true),FILE_APPEND);

   
	$rating_posts  = array();
	$rating_posts_without  = array(); 
	$total = array();
	$totalposts     = $posts;
   // print_r($posts);
	 foreach( $postvisible as $key => $post ) {
		  $rating_posts[] = $post;
			...
			//  $premium_count = count($rating_posts);
			 
		 }
		 $paid = array();
		 $i=0;
		 foreach($rating_posts as $rp2){
			$paid[$i]=$rp2->paid2; 
			$rp = $rp2;
			 unset($rp->paid2);
			 $rating_posts_without[]=$rp;
			$i++;
		 }
		 file_put_contents(ABSPATH."functions.txt","myquery n posts".print_r($posts,true),FILE_APPEND);

	 if(count($rating_posts)) {
		 foreach( $posts as $key => $post ) {
				 if (in_array_r($post, $rating_posts_without) ) {
					 //unset($postvisible[$key] );
				 }
				 else{
					 $total[]=$post;
		 ...
		 file_put_contents(ABSPATH."functions.txt","rating post without".print_r($rating_posts_without,true),FILE_APPEND);
		 $i = 0;
		 foreach($rating_posts as $rp){
			if($paid[$i]){
				$rating_posts[$i]->paid2 = true;
			}
			$i++;
		 }
		 $posts = array_merge($rating_posts, $total);
